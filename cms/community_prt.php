...
<div style="float:left; width:50%; text-align:left; margin-left:5%; font-size:21px;"><?php echo "SPMS/COM/".$acyear_name."/".$row['ref_number']; ?></div>
<div style="float:left; width:40%; text-align:right; margin-right:3%; font-size:21px;">Date: <?=$row['c_date']?></div>
</div>

<h2 style="line-height:46px; font-family:Impact, Haettenschweiler, 'Franklin Gothic Bold', 'Arial Black', sans-serif; font-size:30px;">COMMUNITY CERTIFICATE</h2>
<div style="margin-top:85px;width:866px;margin-left:4px; line-height:60px; text-align:justify;">
<font color="#2E2F65" style="font-size:28px;">This is to certify that <b> <?php echo $row['name']." (".$row['admin_no'].")"; ?> </b>of standard <b><?php echo $row['standard']; ?></b> is a bonafide student of this institution and he/she belongs to <b><?php echo $row['community']; ?></b> community as per the school records. </font></div>
...
$().ready(function() {
	...
	$("#show").change(function(){
		...
		window.location.href='community_prt.php?comid=<?=$comid?>&show=hide';
		}else{
			window.location.href='community_prt.php?comid=<?=$comid?>&show=show';
		}
