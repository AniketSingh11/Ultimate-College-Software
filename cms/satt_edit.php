 ...
 if (isset($_POST['submit']))
{
	...
	$select_record=mysql_query("SELECT * FROM staff WHERE status='1'");
					...
					$staff_id=$queryfetch['staff_id'];
						...
						$lapply1=0;
						$select_record2=mysql_query("SELECT satt_id,l_apply FROM sattendance WHERE st_id='$st_id' AND m_id='$mid' AND day=$eday");
						...
						$sql=mysql_query("UPDATE sattendance SET result='$postTest',lt_id='$ltid',reason='$reason',result_half='$resulthalf',l_apply='$lapply1' WHERE st_id='$st_id' AND m_id='$mid' AND day=$eday");
							}
						}else{
							if(($postTest=='0' || $postTest=='off') && !$apply1){
							...
							$sql=mysql_query("INSERT INTO `sattendance` (`day`,`month`,`year`,`result`,`m_id`,`st_id`,`lt_id`,`reason`,`ay_id`,`result_half`,`l_apply`) VALUES ('$date_day', '$date_month','$date_year', '$postTest','$mid','$st_id','$ltid','$reason','$acyear','$resulthalf','$lapply1')");
						}
					}
		if(!$sql)
{
...
{
 header("Location:satt_edit.php?mid=$mid&eday=$eday&msg=succ");
}
				...
				<div class="block-border">
					...
					<form id="validate-form" class="block-content form" action="" method="post">
						<div class="_25">
							<p>
                                ...
                                <input id="datepicker" name="date1" class="required" type="text" value="<?php echo $eday."/".$mno."/".$year;?>" disabled />
					...
					$stid=$student12['st_id'];?>
                        ...
                        <?php //echo $ssid ." / ".$mid." / ".$eday;
						 $dayatt1=mysql_query("SELECT * FROM sattendance WHERE m_id=$mid AND day=$eday AND st_id=$stid"); 
								  $attday=mysql_fetch_array($dayatt1);
								  $result=$attday['result'];
								  $result_half=$attday['result_half'];
								   $lapply=$attday['l_apply'];
								  ?>
                        <td><center><input type="radio" id="name"  name="<?php echo $student12['st_id']?>" class="text err" value="1" <?php if($result=='1'){ echo 'checked="checked"';}if($lapply){ echo " disabled";}?>/></center></td>
                        <td><center><input type="radio" id="name" name="<?php echo $student12['st_id']?>" class="text err" value="0" <?php if($result=='0'){ echo 'checked="checked"';}if($lapply){ echo " disabled";}?> /></center></td>
                         <td><input type="radio" id="name" name="<?php echo $student12['st_id']?>" class="text err" value="off-M" <?php if($result=='off' && $result_half=="M"){ echo 'checked="checked"';}if($lapply){ echo " disabled";}?> />Morning<br>
                        <input type="radio" id="name" name="<?php echo $student12['st_id']?>" class="text err" value="off-E" <?php if($result=='off' && $result_half=="E"){ echo 'checked="checked"';}if($lapply){ echo " disabled";}?> />Afternoon
                        </td>
                                                <td><?php
											$ltid1=$attday['lt_id'];
                                            ...
                                            <select name="ltid<?php echo $student12['st_id']?>" id="ltid" <?php if($lapply){ echo " disabled";}?>> 
                                            ...
                                            echo '</select>';
                                            ?></td>
                        <td><center><input type="text" id="reason" name="reason<?php echo $student12['st_id']?>" class="text err" value="<?php echo $attday['reason']?>" /></center></td>
                        </tr>
                     <?php $att++; } ?>
