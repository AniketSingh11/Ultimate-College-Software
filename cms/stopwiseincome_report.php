		...
		<div id="main-content">
            ...
            <div class="block-border">
						...
						<div class="_50">
							<p>
								<label for="select">Route Master : <span class="error">*</span></label>
                                	<?php
                                            $result1 = mysql_query("SELECT * FROM trstopping") or die(mysql_error());
                                            echo '<select name="spid" id="spid" class="required"> <option value="">Select Stopping Point</option>';
             ...
             <?php 
					...
					if($spid && $bid){ 
					$stoplist=mysql_query("SELECT * FROM trstopping WHERE stop_id=$spid"); 
					...
					$qry1="SELECT * FROM bfinvoice WHERE sp_id=$spid AND ay_id='" . $acyear. "' AND c_status!='1' AND i_status='0'";
			  ...
			  while($row1=mysql_fetch_array($qry1))
                ...
                <h1> Stopwise Fees Income Report - <?php  echo $stop['stop_name'];?></h1>
                <span style="">Board Name : <strong><?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?></strong></span>
               <span style="margin-left:20px;"><a href="stopwiseincome_export.php?spid=<?php echo $spid;?>&b_id=<?php echo $bid;?>&ayid=<?php echo $acyear;?>" style="width:100px"><button class="btn btn-small btn-green"><img src="img/icons/packs/fugue/16x16/inbox-download.png"> Download Report</button></a></span>
				...
				<div class="block-border">
					<div class="block-header">
                    	<h1> Stopwise Fees Income Report (<?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?>)</h1>                       
                        <span></span>
					</div>
					<div class="block-content">
						<table id="table-example" class="table">
							...
							<tbody>
                            <?php 
							$qry="SELECT * FROM bfinvoice WHERE sp_id=$spid AND ay_id=$acyear AND c_status!='1' AND i_status='0'";
