...
<a href="monthly_salary_prt_full.php?m=<?php echo $m_value."&syear=".$syear."&eyear=".$eyear."&acid=".$acyear;?>" title="Print Report"><button type="button" class="btn btn-warining" style="font-size: 13px;">Print FullReport</button></a>								 
        ...
        ?>			
        <?php	
											$emp_query="select t1.others_id,t1.fname,t1.category_id,t1.email,t1.phone_no,t1.position from others t1 LEFT JOIN staff_month_salary t2 ON t1.o_id = t2.o_id AND t2.month=$m_value AND t2.year=$y_value WHERE t2.o_id IS NULL AND t1.status='1' AND t1.s_type='0'";
							$emp_result=mysql_query($emp_query);
							while($emp_display=mysql_fetch_array($emp_result))
							{
        ...
        ?>		
        <?php	
							$emp_query="select t1.driver_id,t1.fname,t1.d_type,t1.email,t1.phone_no,t1.position from driver t1 LEFT JOIN staff_month_salary t2 ON t1.d_id = t2.d_id AND t2.month=$m_value AND t2.year=$y_value WHERE t2.d_id IS NULL AND t1.status='1' AND t1.s_type='0'";
							$emp_result=mysql_query($emp_query);
							while($emp_display=mysql_fetch_array($emp_result))
							{
