...
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Loan Payment Details");
...
$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(20);cellFont("A3","Calibri");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Date");$column++;
$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(20);cellFont("B3","Calibri");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Pay Amount");$column++;

$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(20);cellFont("C3","Calibri");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Total Payment");$column++;

$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);cellFont("D3","Calibri");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Balance");$column++;
...
while($emp_display = mysql_fetch_array($emp_result))
{
    $column = 'A';
			...
			$balance=$lpay-$total_pay;
							    ...
							    $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $emp_display["date"]);  $column++;
							    $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $emp_display["amount"]);  $column++;
							    $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $total_pay); $column++;
							    $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $balance); $column++;
							    
							    $rowCount++;
										$content .= stripslashes($count). ',';
										$content .= stripslashes($emp_display["date"]). ',';
										$content .= stripslashes($emp_display["amount"]). ',';
										$content .= stripslashes($total_pay). ',';
										$content .= stripslashes($balance). ',';
										$content .= "\n";
							$count++;	
}

$objPHPExcel->getActiveSheet()->setTitle("Loan Payment Report");
$ptypename=str_replace(" ","",$ptypename);
// Redirect output to a client’s web browser (Excel5)
header('Content-Type: application/vnd.ms-excel');
header("Content-Disposition: attachment;filename=Loan-payment-report.xls");
