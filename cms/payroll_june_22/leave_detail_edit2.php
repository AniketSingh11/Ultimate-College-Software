...
$employee=mysql_fetch_array($emp_result);
  ...
  {
	$sid= mysql_real_escape_string($_POST["d_id"]);
	...
	if(($l_total>$re_leave) && $re_leave!="other"){
		header("location:leave_detail_edit2.php?id=$id&msg=ltotal");
		exit;
	}
	$query="update staff_leave set d_id='$sid',staff_id='$staff_id', staff_name='$staff_name', l_type='$l_type', l_type_name='$l_type_name', a_date='$a_date', f_date='$f_date', t_date='$t_date', l_total='$l_total', l_des='$l_dec', h_type='$h_type', day='$sdate_day', month='$sdate_month', year='$sdate_year', f_day='$f_day', f_month='$f_month', f_year='$f_year', t_day='$t_day', t_month='$t_month', t_year='$t_year', status='$status', from_date='$from_date', to_date='$to_date' where id='$id'";
	$result=mysql_query($query);
	
	if($result)
	{	
		header("location:leave_detail_edit2.php?id=$id&msg=succ");
	}
	else
	{
		header("location:leave_detail_edit2.php?id=$id&msg=err");
    }	
  }
   ?>
  </head>
...
$employee11=mysql_fetch_array($emp_result11);
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1><?php echo $employee11['fname']." ".$employee11['lname'];?> - Leave Edit <a href="leave_detail2.php?id=<?php echo $s_id;?>&syear=<?php echo $ay['s_year'].'&eyear='.$ay['e_year'];?>"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
...
<div class="col-sm-6">
								<div class="form-group">
									...
									<input type="text" id="staff_id" name="staff_id" class="form-control" data-required="true" value="<?php echo $employee11['driver_id'];?>" readonly>
								</div>								
                                <div class="form-group">
									...
									<select id="l_type" name="l_type" class="form-control" data-required="true">
										...
										$emp_query12="select * from staff_leave where status='1' AND (d_id=$stid AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";	
										...
										while($emp_display1=mysql_fetch_array($emp_result1))
										{
											...
											$tleave=0;
											$emp_query2="select * from staff_leave where status='1' AND (d_id=$stid  AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";	
...
<option value="1" <?php if($l_status == 1){ echo 'selected'; } ?>  >Approved</option>							
                    ...
                    <center>
<div class="form-group">
								<input type="hidden" name="d_id" value="<?php echo  $s_id;?>"/>
...
$(document).ready(function(){
    $("#l_type").change(function(){
		var thiss = $(this);
		var value = thiss.val(); 
        $.get("leave_calculate.php",{value:value, stid:<?php echo $s_id;?>, type:'dr'},function(data){
			$( "#test" ).html(data);
        });
    });
});
