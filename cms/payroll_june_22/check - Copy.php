...
for($i=1;$i<=31;$i++){
	$fdate=$sdate_year."-".$sdate_month."-".str_pad($i, 2, "0", STR_PAD_LEFT);
 $emp_query1="select * from staff_leave where (from_date<='$fdate' AND to_date>='$fdate') AND (o_id='$s_id' AND status='1')";
										$emp_result1=mysql_query($emp_query1);
					$emp_display1=mysql_fetch_array($emp_result1);
					if($emp_display1){
						$ltype=$emp_display1['l_type'];
...
$loanpaylist=mysql_query("select * from staff_loan_pay WHERE o_id=$s_id AND l_id='$l_id' order by lp_id desc");
$loanpay1=mysql_fetch_array($loanpaylist);
