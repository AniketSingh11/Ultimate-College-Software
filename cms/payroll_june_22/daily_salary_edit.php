  ...
  if(isset($_POST["submit"]))
  {
	  $salary_date1= mysql_real_escape_string($_POST["salary_date1"]);
		  ...
		  $sdate_year1=$sdate_split2[2];
		   ...
		   $qry="UPDATE staff_daily_salary SET date_day='$sdate_day1',date_month='$sdate_month1',date_year='$sdate_year1' WHERE st_ms_id='$emp_id'";
$result = mysql_query($qry) or die("Could not edit data into DB: " . mysql_error());

if($result){
	header("location:daily_salary_edit.php?id=$emp_id&stid=$s_id&type=$type&msg=succ");	
}else{
	header("location:daily_salary_edit.php?id=$emp_id&stid=$s_id&type=$type&msg=err");	
}
		  }
   ?>
  </head>
 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1> Employee Salary Add <a href="daily_salary.php?m=<?php echo date("m");?>&syear=<?php echo $ay['s_year'].'&eyear='.$ay['e_year'];?>"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
