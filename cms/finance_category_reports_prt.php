...
$lastyear=$_GET['lastyear'];
                                            ...
                                            endwhile;
                                            echo '</select>';
                                            ?><br><a href="finance_category_excel.php?excid=<?php echo $gexcid; ?>&lastyear=<?php echo $lastyear; ?>" style="background: #6DA42B;
...
<h2 style="line-height:46px; font-family:Impact, Haettenschweiler, 'Franklin Gothic Bold', 'Arial Black', sans-serif; font-size:30px;">CATEGORY WISE FINANCE REPORTS</h2>
			  ...
			  while($agency=mysql_fetch_assoc($agencylist))
        		{
					...
					$tpamount=0;
							...
							if($excid){
								$qry2 .=" WHERE exc_id=$excid AND ay_id=$acyear";
							}
...
$().ready(function() {
	$("#show").change(function(){
		...
		window.location.href='finance_category_reports_prt.php?show=hide';
		}else{
			window.location.href='finance_category_reports_prt.php?lastyear=<?=$lastyear?>&excid=<?=$gexcid?>&show=show';
		}	
	});	
	$("#lastyear").change(function(){
		if(this.checked) {
		window.location.href='finance_category_reports_prt.php?lastyear=1&excid=<?=$gexcid?>';
		}else{
			window.location.href='finance_category_reports_prt.php?excid=<?=$gexcid?>';
		}	
	});	
	$("#show1").change(function(){
		var a=this.value;
		window.location.href='finance_category_reports_prt.php?lastyear=<?=$lastyear?>&excid='+a;
	});	
});
