...
header("Expires: 0");
	 ...
	 $qry="SELECT * FROM finvoice WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate. "' AND ay_id='$ayid' ";
...
if($bid=='All'){
    ...
    $qry="SELECT * FROM finvoice WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate. "' AND ay_id='$ayid' and c_status!='1' ";
    if(!empty($bid) && $bid!='All') { $qry .= " AND bid = '" . $bid. "'"; }
...
$boardlist1=mysql_query("SELECT * FROM board WHERE b_id=$bid1"); 
	...
	$fi_name= stripslashes($student["admission_number"]);
...
$objPHPExcel->getActiveSheet()->setTitle("Fees Income Report");
...
header('Content-Disposition: attachment;filename="FeesIncome_report-list.xls"');
