    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
				...
				<li class="no-hover">Driver Attendance Management</li>                
			</ul>
		...
		<div class="grid_12">
				<h1>Driver Attendance <b><?php if($mid){?>( <?php echo $month['m_name'];?> )<?php } ?></b></h1>
                 <?php 
			  ...
			  while($row1=mysql_fetch_array($qry1))
        		{
					$mno=$row1['m_no'];	
					if($mcount==1){?>
                 <a href="datt_mng.php?mid=<?php echo $row1['m_id']; ?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-error" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> <?php echo $row1['m_name']; ?></button></a>
                <?php }
                ...
                <br>
                <?php if($mid){?>
                <a href="datt_single.php?mid=<?php echo $mid;?>" title="add" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png"> Add Attendance</button></a>
            ...
            <?php if($mid){
				$qry1=mysql_query("SELECT distinct day FROM sattendance WHERE m_id=$mid AND ay_id=$acyear AND d_id ORDER BY day");
            	...
            	<div class="block-border">
						...
						<table id="table-example" class="table">
							<thead>
								<tr>
                                    ...
                                    <?php 
						$select_record2=mysql_query("SELECT distinct day FROM sattendance WHERE m_id=$mid AND ay_id=$acyear AND d_id ORDER BY day");
					...
					{ 
                        ...
                        <?php } ?>
							...
							<tbody>                    			
                            <?php 
							//$qry=mysql_query("SELECT distinct d_id FROM sattendance WHERE m_id=$mid AND ay_id=$acyear");
							$qry=mysql_query("SELECT * FROM driver WHERE status='1'");
							$count=1;
			  while($row=mysql_fetch_array($qry))
        		{
						$did=$row['d_id'];
								   //$studentlist=mysql_query("SELECT * FROM staff WHERE d_id=$did"); 
					...
					{ 
						$day=$monthday['day'];
						$select_re=mysql_query("SELECT * FROM sattendance WHERE d_id=$did AND m_id=$mid AND day=$day AND ay_id=$acyear ORDER BY day");
                ...
                <p><b>Reason  </b>: <?php echo $attend['reason'];?></p>				
                           ...
                           <?php if($count!=1){?> 
                            <tr class="gradeX">
                                    ...
                                    <?php 
						$select_recor=mysql_query("SELECT distinct day FROM sattendance WHERE m_id=$mid AND ay_id=$acyear AND d_id ORDER BY day");
					...
					$eyear=$monthday1['year'];
						?>
                        <td><center><a href="datt_edit.php?mid=<?php echo $mid;?>&eday=<?php echo $eday;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a> 
                        <a href="datt_delete.php?mid=<?php echo $mid;?>&eday=<?php echo $eday;?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a></center>
                        </td>
                        <?php } ?>
								</tr>  <?php } ?>                        																
							</tbody>
