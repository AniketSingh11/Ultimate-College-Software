                            ...
                            <div id="invoice" class="widget widget-plain">			
                                ...
                                <div class="widget-content">
                                    <table class="table table-striped" id="table-example">	
                                        ...
                                        $bamountthisyear = 0;
                                        $bus_lastyear = 0;
                                        $booklist1 = mysql_query("SELECT fi_total,pending FROM bfinvoice WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate . "' AND '" . $enddate . "' AND ay_id=$acyear AND c_status!='1' AND i_status='0'");
                                        ...
                                        while ($row6 = mysql_fetch_assoc($qry6)) {
                                            ...
                                            while ($exrow = mysql_fetch_assoc($exsqry)) {
                                                ...
                                                if ($count1 == 0) {
                                                    ...
                                                    //other expense
                                                    $exc_amount = 0;
                                                    $feeslist1 = mysql_query("SELECT status,amount,pending,advance_status,advance_amt FROM exponses WHERE (date_year*10000) + (date_month*100) + date_day between '" . $startdate . "' AND '" . $enddate . "' AND exc_id=$exc_id  AND  exs_id IN (" . implode(',', $myarray) . ") AND type=0");
                                                    while ($fees1 = mysql_fetch_assoc($feeslist1)) {
                                                        $type = $fees1['type'];
                                                        $status = $fees1['status'];
                                                        $amount1 = ($fees1['amount']+$fees1['advance_amt']); // advance amount adding
                                                ...
                                                while ($exrow = mysql_fetch_array($exsqry)) {
                                                    ...
                                                    if ($count1 == 0) {
                                                        ...
                                                        while ($fees1 = mysql_fetch_array($feeslist1)) {
                                                            $exid = $fees1['ex_id'];
                                                            $amount1 = ($fees1['amount']+$fees1['advance_amt']);      /*advance amt add*/
                                                            $amountpaid = 0;
                                                            $pending = 0;
