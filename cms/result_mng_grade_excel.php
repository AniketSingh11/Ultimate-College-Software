					...
					$ssid=$student['ss_id'];
						...
						if($nofrow>1){
							...
							$count=1;
							  while($row1=mysql_fetch_assoc($qry1))
								{
							$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, 'FA');cellFont($column.$rowCount,"Cambria");  $column++;
							$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, 'SA');cellFont($column.$rowCount,"Cambria");  $column++;	
							$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, 'Total');cellFont($column.$rowCount,"Cambria");  $column++;
								}
							$rowCount++;
							...
							$count=1;
							  while($row1=mysql_fetch_assoc($qry1))
								{
										$content .= stripslashes('FA'). ',';
										$content .= stripslashes('SA'). ',';
					...
					$column = 'A';	
							...
							$gtotal=0;
							  while($row1=mysql_fetch_assoc($qry1))
								...
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $fa_grade); $column++;	
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $sa_grade); $column++;
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $fa_sa_grade); $column++;
								$gtotal +=$total; }
								}
								...
								if($subjectype=='1'){					
								 ...
								 if($paper=='1'){
									...
									if($fa_sa_grade=="E"){
										...
										$content .= stripslashes($fa_grade). ',';
										$content .= stripslashes($sa_grade). ',';
										$content .= stripslashes($fa_sa_grade). ',';
										$gtotal +=$total; }
								}
...
$rowCount++;
	...
	/*****************************************sheet 2 *************************************************/
$name=$class['c_name']."-".$section['s_name']."-".$exam['e_name']."-Grade";
