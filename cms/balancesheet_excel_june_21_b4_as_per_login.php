...
/********************** TC Issued charges ******************************************************/
     ...
     $other1=mysql_query("SELECT * FROM tc_xi_kg WHERE (date_year*10000) + (date_month*100) + date_day between '" . $startdate. "' AND '" . $enddate. "' AND ay_id='" . $acyear. "'");
                                        $tc=0;
...
if ($bamountthisyear != 0) {
    ...
    $objWorkSheet->setCellValue($column . $rowCount, $count);
    $column++;
    $objWorkSheet->setCellValue($column . $rowCount, 'Bus Fees('.$frmmto.')');
    $column++;
    $objWorkSheet->setCellValue($column . $rowCount, '-');
    $column++;
