...
error_reporting(E_ALL ^ E_NOTICE);
 ...
 if(isset($_POST['place-order'])){
	...
	$invoicelist=mysql_query("SELECT * FROM finvoice WHERE fi_id=$id"); 
			  $exbill=mysql_fetch_array($invoicelist);
			$oldamount=$exbill['fi_total'];
			$oldcstatus=$exbill['c_status'];
			$ba_id=$exbill['ba_id'];
     $qry1=mysql_query("UPDATE finvoice  SET fi_ptype='$ptype',ba_id='$baid',pay_number='$pay_number',c_status='$c_status',receive_cdate='$cdate',receive_cbank='$cbank'  WHERE fi_id='$id'") or die(mysql_error());
	 if($c_status=='1'){
...
$result1 = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
    ...
    header("location:chequefeesinvoice_detail.php?fiid=$id&bid=$bid&msg=dsucc");
 ...
 <!--! end of #search-bar -->
    ...
    <div id="main" role="main">
		...
		<div id="main-content">
            ...
            <a href="cheque_feesinvoice.php?bid=<?php echo $bid;?>" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a>
			...
			?>
                        ...
                        <tr>
                            ...
                            <div class="_25">
                                    <p>
                                        ...
                                        <input  name="receive_cdate" id="cdate" style="width: 206px;" type="text" value="<?php echo $invoice['receive_cdate']?>" />
                                    </p>
                                    </div>
                            <span id="pstatus">
                            <?php 
			  ...
			  $depositlist=mysql_query("SELECT * FROM bank_deposit WHERE fi_id=$ffi_id"); 
               ...
               <input type="button" value="Print Invoice" name="Print" onClick="window.open('finvoice_prt.php?fiid=<?php echo $ffi_id; ?>', '_blank');" class="btn btn-green" style="width:120px">&nbsp;&nbsp;
                </td>
                </tr></form>
					</tbody>
                  </table>				
