 ...
 if (isset($_POST['submit']))
{
	 ...
	 $r_contact=$_POST['r_contact'];
		...
		$sql="INSERT INTO route (r_name,v_id,d_id,sd_id,status,r_contact) VALUES
('$r_name','$v_id','$d_id','$sd_id','1','r_contact')";

$result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
    if($result){
        header("Location:route_add.php?msg=succ");
    }
    exit;
}
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Add New Route Master</li>
			</ul>
            ...
            <?php } ?>
				...
				<div class="block-border">
					<div class="block-header">
						<h1>Add New Route Master</h1><span></span>
					</div>
					<form id="validate-form" class="block-content form" action="" method="post">
						<div class="_100">
							<p>
                                <label for="textfield">Route Master Name</label>
                                <input id="textfield" name="r_name" class="required" type="text" value="" />
                            </p>
						</div>
                        <div class="_50">
							<p>
								<label for="select">Vehicle No : (display only not assign Vehicles)<span class="error">*</span></label>
                                	<?php
                                            $classl = "SELECT v_id,v_no FROM vehicle";
                                            $result1 = mysql_query($classl) or die(mysql_error());
                                            echo '<select name="v_id" id="v_id" class="required"> <option value="">Select Vehicle No</option>';
							...
							<p>
								<label for="select">Driver: <span class="error">*</span></label>
                                	<?php
                                            ...
                                            $result1 = mysql_query($classl) or die(mysql_error());
                                            echo '<select name="d_id" id="d_id" class="required"> <option value="">Select Driver</option>';
											while ($row1 = mysql_fetch_assoc($result1)):
							...
							<p>
								<label for="select">Bus Assistant: <span class="error">*</span></label>
                                	<?php
                                            ...
                                            $result1 = mysql_query($classl) or die(mysql_error());
                                            echo '<select name="sd_id" id="sd_id" class="required"> <option value="">Select Bus Assistants</option>';
											while ($row1 = mysql_fetch_assoc($result1)):
                                                echo "<option value='{$row1['d_id']}'>{$row1['fname']} {$row1['lname']}</option>\n";
                                            endwhile;
                                            echo '</select>';
                        ...
                        <div class="_25">
							<p>
                                ...
                                <input id="r_contact" name="r_contact"  type="text" value="" class="required"/>
                            </p>
						</div>
						<div class="clear"></div>
