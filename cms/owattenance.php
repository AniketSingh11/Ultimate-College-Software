...
if( (isset ($_GET['value']) && $_GET['value']!=''))
{
  ...
  echo '<div class="_100">
                    ...
                    <tbody>';
						 ...
						 $att=1;						 
					$select_record=mysql_query("SELECT * FROM others WHERE status='1' order by fname asc");
					while($student12=mysql_fetch_array($select_record))
					{ 
					$o_id=$student12['o_id'];
					
					$emp_query="select * from staff_leave where (from_date<='$finaldate' AND to_date>='$finaldate') AND o_id=$o_id AND status='1'";
                    ...
                    echo '<tr>
                        ...
                        <td><center>'.$student12['others_id'].'</center></td>
                        <td><center>'.$student12['fname']." ".$student12['lname'].'</center></td>
                        <td><center><input type="radio" id="name"  name="'.$student12['o_id'].'" class="text err" value="1"';
						...
						echo 'disabled';}
						echo '/></center></td>
                        <td><center><input type="radio" id="name" name="'.$student12['o_id'].'" class="text err" value="0"';
						...
						echo 'disabled';}
						echo '/></center></td>
                        <td><input type="radio" id="name" name="'.$student12['o_id'].'" class="text err" value="off-M"';
						if($morning){
							$leave="off-M";
						echo 'checked="checked"';}
						if($emp_display){
						echo 'disabled';}
						echo '/>Morning<br>
                            <input type="radio" id="name" name="'.$student12['o_id'].'" class="text err" value="off-E"';
						if($evening){
							$leave="off-E";
						echo 'checked="checked"';}
						if($emp_display){
						...
						echo '/>Afternoon';
							if($emp_display){
								echo '<input type="hidden" id="ltid" name="'.$student12['o_id'].'" class="text err" value="'.$leave.'" />';
							}
                        echo '</td>
                        <td>';
								...
								echo '<select name="ltid'.$student12['o_id'].'" id="ltid"';
								 	if($ltid){
									echo 'disabled';}
                                      echo '><option value="">Select Leave Type</option>';
                                        ...
                                        echo '</select>';
										if($ltid){
									echo '<input type="hidden" id="ltid" name="ltid'.$student12['o_id'].'" class="text err" value="'.$ltid.'" />';}
									echo '<input type="hidden" id="apply" name="apply'.$student12['o_id'].'" class="text err" value="';
									if($staffleaveid){ echo $staffleaveid;}else { echo "0"; }
									echo '" />';
										echo '</td>
                        <td><center><input type="text" id="reason" name="reason'.$student12['o_id'].'" class="text err" value="'.$emp_display['l_des'].'" /></center></td>                        </tr>';
                     $att++; }
                   echo '</tbody>
                  </table></p><br><br>	                  	
        			</div>
