...
$did=$_GET['did'];
  ...
  foreach($_POST['deduction'] as $value1)
  {
	...
	$query=mysql_query("update staff_salary set salary='$salary',allowance='$allowance',deduction='$deduction',status='$status',extra_salary='$ex_salary' where id=$id");
	if($query)
	{
		header("location:driver_salary_edit.php?id=$id&did=$did&msg=succ");		
	}
	else
	{
		header("location:driver_salary_edit.php?id=$id&did=$did&msg=err");		
	}
 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1> <?php echo $staffs['fname']." ".$staffs['lname'];?> - Salary Details Edit <a href="driver_salary_list.php?id=<?php echo $did;?>"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
...
<div class="col-sm-6">
							...
							$allowance = explode( ',', $salarylist["allowance"]);	
				                ...
				                <div class="checkbox">
									<label>
									    <input type="checkbox" name="allowance[]" class="" <?php if($allow=='1'){ echo "checked";}?>  value="<?php echo $emp_display["id"]; ?>" data-required="true" data-mincheck="2">
