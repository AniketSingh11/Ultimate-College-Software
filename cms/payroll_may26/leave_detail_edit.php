...
$employee=mysql_fetch_array($emp_result);
  ...
  if(isset($_POST["submit"]))
  {
	...
	if(($l_total>$re_leave) && $re_leave!="other"){
		header("location:leave_detail_edit.php?id=$id&msg=ltotal");
		exit;
	}
...
<div class="col-sm-6">
                                ...
                                <div class="form-group">
									...
									<select id="l_type" name="l_type" class="form-control" data-required="true">
										...
										$emp_query12="select * from staff_leave where status='1' AND (st_id=$stid AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";	
										...
										while($emp_display1=mysql_fetch_array($emp_result1))
										{
											...
											$tleave=0;
											$emp_query2="select * from staff_leave where status='1' AND (st_id=$stid  AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";	
...
$(document).ready(function(){
    $("#l_type").change(function(){
		var thiss = $(this);
		var value = thiss.val(); 
        $.get("leave_calculate.php",{value:value, stid:<?php echo $s_id;?>, type:'st'},function(data){
			$( "#test" ).html(data);
        });
    });
});
