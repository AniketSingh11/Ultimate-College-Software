...
$query="insert into staff_daily_salary(st_id,o_id,d_id,staff_name,staff_id,date,day,month,year,g_salary,n_salary,d_total,doj,position,role,ay_id,date_day,date_month,date_year)values('$stid','$oid','$did','$staff_name','$staff_id','$salary_date','$sdate_day','$sdate_month','$sdate_year','$gross_total','$net_salary','$deduction_total','$doj','$position','$role','$acyear','$sdate_day1','$sdate_month1','$sdate_year1')";
//die();
//echo $query;die;
...
$d_d=date('d');						  
								  ...
								  $emp_query="select * from others WHERE status='1' and s_type='1' order by fname asc";
...
$d_d=date('d');	
								//  $emp_query="select * from driver WHERE status='1' and s_type='1' and d_id not in (select d_id FROM staff_daily_salary WHERE month =$month and year=$yr)";
								  $emp_query="select * from driver WHERE status='1' and s_type='1'";
										$emp_result=mysql_query($emp_query);
										while($emp_display=mysql_fetch_array($emp_result))
										{
