...
$employee=mysql_fetch_array($emp_result);
  ...
  if(isset($_POST["submit"]))
  {
	...
	$query=mysql_query("select lt_name from leavetype where lt_id='$l_type' ");
	...
	if(($l_total>$re_leave) && $re_leave!="other"){
		header("location:leave_edit.php?id=$id&msg=ltotal");
		exit;
	}
	...
	if($result)
	{	
		header("location:leave_edit.php?id=$id&msg=succ");
	}
	else
	{
		header("location:leave_edit.php?id=$id&msg=err");
    }	
  }
   ?>
  </head>
  ...
  $h_type=$employee["h_type"];
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1><?php echo $employee11['fname']." ".$employee11['lname'];?> - Leave Edit <a href="leave_list.php?syear=<?php echo $ay['s_year'].'&eyear='.$ay['e_year'];?>"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
...
<form id="validate-enhanced" action="" class="form parsley-form" method="post" enctype="multipart/form-data" name="first">
     ...
     <div class="col-sm-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div id="dp-ex-2" class="input-group date ui-datepicker1" style="width:75%" data-date-format="dd-mm-yyyy">
		                                <input id="date" name="date" class="form-control" type="text" data-required="true" value="<?php echo $employee['a_date'];?>">
...
<div class="col-sm-6">
                                ...
                                <div class="form-group">
									...
									<select id="l_type" name="l_type" class="form-control" data-required="true">
										...
										$emp_result=mysql_query("select other,l_total from leavetype where lt_id=$l_type1");
										$emp_display=mysql_fetch_array($emp_result);
										$tother=$emp_display["other"];
										$emp_query12="select l_total from staff_leave where status='1' AND ((st_id=$stid AND o_id=$oid AND d_id=$did) AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";	
