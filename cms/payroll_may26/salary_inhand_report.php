...
<?php } ?>	 <div class="row">
	...
	//$emp_result1=mysql_query("SELECT a.n_salary,  b.salarytype FROM staff_month_salary a, staff b   WHERE b.salarytpe=1 AND a.id = b.st_id");
	
//$emp_result1=mysql_query("select * from staff_month_salary inner join staff where salarytype='1' and staff_month_salary.month=$month and staff_month_salary.year=$year");
									
  			
									$emp_query1="select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_month_salary where month=$month and year=$year order by st_ms_id desc";	
							//echo "select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_month_salary where month=$month and year=$year order by st_ms_id desc";	die;									
							...
							while($emp_display1=mysql_fetch_array($emp_result1))
							{
								...
								if($st_id && mysql_num_rows(mysql_query("SELECT st_id FROM staff WHERE salarytype=1 AND st_id=$st_id"))){
									
									$bank=1;
								}
								//echo "SELECT st_id FROM staff WHERE salarytype=1 AND st_id=$st_id"; die;
								$o_id=$emp_display1["o_id"];	
								if($o_id && mysql_num_rows(mysql_query("SELECT o_id FROM others WHERE salarytype=1 AND o_id=$o_id"))){
									$bank=1;
								}
								$d_id=$emp_display1["d_id"];	
								if($d_id && mysql_num_rows(mysql_query("SELECT d_id FROM driver WHERE salarytype=1 AND d_id=$d_id"))){
									$bank=1;
								}
								if($bank=='1'){
									$total +=round($emp_display1["n_salary"]); } }
										...
										$emp_query1="select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_daily_salary where month=$month and year=$year order by st_ms_id desc";	
							//echo "select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_month_salary where month=$month and year=$year order by st_ms_id desc";	die;									
							$emp_result1=mysql_query($emp_query1);
							...
							while($emp_display1=mysql_fetch_array($emp_result1))
							{
								...
								$bank=0;
								$st_id=$emp_display1["st_id"];
								if($st_id && mysql_num_rows(mysql_query("SELECT st_id FROM staff WHERE s_type=1 AND st_id=$st_id"))){
									
									$bank=1;
								}
								//echo "SELECT st_id FROM staff WHERE salarytype=1 AND st_id=$st_id"; die;
								$o_id=$emp_display1["o_id"];	
								if($o_id && mysql_num_rows(mysql_query("SELECT o_id FROM others WHERE s_type=1 AND o_id=$o_id"))){
									$bank=1;
								}
								$d_id=$emp_display1["d_id"];	
								if($d_id && mysql_num_rows(mysql_query("SELECT d_id FROM driver WHERE s_type=1 AND d_id=$d_id"))){
									$bank=1;
								}
								if($bank=='1'){
