...
('$s_code','$s_name','$cid','$sid')";*/
	...
	$sql="UPDATE subject SET sl_id='$slid',st_id='$stid',b_id='$bid',ay_id='$acyear' WHERE sub_id='$subid'";

$result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
    if($result){
        header("Location:subjectmng_edit1.php?cid=$cid&sid=$sid&subid=$subid&bid=$bid&msg=succ");
    }
    exit;
}
    ...
    <div id="main" role="main">
    				<?php 
							...
							$sectionlist=mysql_query("SELECT * FROM section WHERE s_id=$sid"); 
								  ...
								  $row=mysql_fetch_assoc($subjectlist);
								  $stid=$row['st_id'];	
								  $slid=$row['sl_id'];	
							$bid=$_GET['bid'];
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1>Edit Staff details (<?php echo $class['c_name']."-".$section['s_name'];?>)</h1>                
						...
						<div class="_100">
									...
									<option value='<?php echo $row2['sl_id'];?>' selected><?php echo $row2['s_name'];?></option>
									 <?php } else{
										 $slid1=$row2['sl_id'];
												...
												if($subcheck=='0'){?>
									 <option value='<?php echo $row2['sl_id'];?>'><?php echo $row2['s_name'];?></option>                         					<?php  } }
								endwhile;
								echo '</select>';
								?>
								</select>
                         ...
                         <div class="_100">
							<p>
                                ...
                                <?php
                                $classl1 = "SELECT * FROM staff WHERE s_type='Teaching'";
                                $result1 = mysql_query($classl1) or die(mysql_error());
                                echo '<select name="stid" id="stid"> 
                                <option value="">Select Staff</option>';
                                while ($row1 = mysql_fetch_assoc($result1)): 
                                if($row1['st_id']==$stid){?>
                                    
                                    <option value='<?php echo $row1['st_id'];?>' selected><?php echo $row1['fname']." ".$row1['mname']." ".$row1['lname'];?></option>
                                    <?php } else{?>
