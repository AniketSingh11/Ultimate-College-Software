 ...
 if (isset($_POST['submit']))
{
	...
	//die();
		...
		if($image[error]!=4){	
    		...
    		header("Location: circular1_edit.php?clid=$clid&msg=eronfile");
    		exit;
			}
		...
		unlink("circular/".$file);
				...
				$sql="UPDATE circular SET cl_day='$publish_day',cl_month='$publish_month',cl_year='$publish_year',title='$title',descript='$descript',type='Student',file='$newfilename',status='$status',b_id='$bid',c_id='$cid',s_id='$sid' WHERE cl_id='$clid'";
				
				$result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
				if($result){
					 header("Location:circular1_edit.php?clid=$clid&msg=succ");
				}
			} else {
			...
			$sql="UPDATE circular SET cl_day='$publish_day',cl_month='$publish_month',cl_year='$publish_year',title='$title',descript='$descript',type='Student',status='$status',b_id='$bid',c_id='$cid',s_id='$sid' WHERE cl_id='$clid'";
							...
							<p>
								<label for="select">Standard : <span class="error">*</span></label>
                                	<select name="cid" id="cid" class="required" onchange="showCategory(this.value)">
											<?php if($circular['c_id']){
												$cid=$circular['c_id'];
												$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid"); 
								  $class=mysql_fetch_array($classlist);?>
                                <option value="<?php echo $cid;?>" selected><?php echo $class['c_name'];?></option>
                                <?php } ?>											
								</select>
							</p>
						</div>
                        <div class="_50">
							<p>
                               ...
                               <select name="sid" id="sid">
											...
											<?php if($circular['s_id']!=""){
											    $sid=$circular['s_id'];
											     
												
												$classlist=mysql_query("SELECT * FROM section WHERE c_id=$cid"); 
								  while($class=mysql_fetch_array($classlist))
								  {?>
                                <option value="<?php echo $class["s_id"];?>" <?php if($class["s_id"]==$sid){?>selected<?php }?>><?php echo $class['s_name'];?></option>
                                <?php }} ?>											
								</select>
							</p>
						</div>
