...
{ 
	  ...
	  //$seid=$_POST['se_id'];
	   ...
	   if($poor_pay_amount && $fund_amount){
		  ...
		  $cat=$category;
	   $qry31=mysql_query("SELECT * FROM finvoice_no WHERE category='$category'and ay_id=$ay_id"); 
	   ...
	   if($category=="CH")
	   	 $category="HSS";
	   $invoice_no=$category.str_pad($invoice_no1, 3, '0', STR_PAD_LEFT);
	 $termfees=$_POST['termfees'];
	 $cheque_service=$_POST['cheque_service'];
				  
	 $sql="INSERT INTO finvoice (fr_no,fi_name,fi_total,fi_ptype,fi_day,fi_month,fi_year,ss_id,c_id,s_id,stype,fi_by,bid,ay_id,pay_number,get_amount,balance,poor_student_pay,fund_amount,funds,bank_name,cheque_date,receive_cdate,receive_cbank,cheque_service,cat) VALUES
('$invoice_no','$ss_name','$total','$ptype','$day','$month','$year','$ssid1','$cid1','$sid1','$stype','$user','$bid1','$acyear','$pay_number','$get_amount','$balance','$poor_pay_amount','$fund_amount','$fund','$bank_name','$cheque_date','$cdate','$cbank','$cheque_service','$cat')";
$update_cheque=mysql_query("update finvoice set service_charge_status=1 where fi_id=$c_fi_id");
	   ...
	   if($category=="HSS")
	   	 $category="CH";
			 $inovoice=$invoice_no1+1;
					$qry1=mysql_query("UPDATE finvoice_no SET count='$inovoice' WHERE category='$category'and ay_id=$ay_id");
									  ...
									  while($row2=mysql_fetch_array($qry2))
										{
											...
											if(product_exists_visible($cartid)==1 || $fdiscount){
												...
												$payment=0;
												 ...
												 if($pending){
											$sql1="INSERT INTO fsalessumarry (fi_id,fr_id,fg_id,fgd_id,name,amount,tamount,pamount,type,payment,discount,dv_id,bid,ay_id,discount_remark) VALUES
('$lastid','$frid','$ffg_id','$fgdid','$name','$amount','$rate','$pamount','other','$payment','$fdiscount','$dvid','$bid1','$acyear','$discount_remark')";
								...
								$ffg_id=$student12['fg_id'];
									...
									if($termfees>1){
										  ...
										  if(product_exists_visible($cartid)==1 || $fdiscount){
												...
												$sql1="INSERT INTO fsalessumarry (fi_id,fr_id,fg_id,fgd_id,name,amount,tamount,pamount,type,payment,discount,dv_id,bid,ay_id,discount_remark) VALUES
('$lastid','$frid','$ffg_id','$fgdid','$name','$amount','$rate','$pamount','terms','$payment','$fdiscount','$dvid','$bid1','$acyear','$discount_remark')";
					$result1 = mysql_query($sql1) or die("Could not insert data into DB: " . mysql_error());
     	...
     	/********************************************************Other Extra fees ************************************************/
			$max=count($_SESSION['tfees']);
					...
					$fgrqry=mysql_query("SELECT name FROM fgroup_detail WHERE fgd_id=$q"); 
								  ...
								  if($w){
									echo $sql1="INSERT INTO fsalessumarry (fi_id,fr_id,fg_id,fgd_id,name,amount,tamount,pamount,type,payment,bid,ay_id,discount_remark) VALUES
('$lastid','','$r','$q','$name','$t','$t','0','otherfees','0','$bid1','$acyear','$discount_remark')";
					$result1 = mysql_query($sql1) or die("Could not insert data into DB: " . mysql_error());
												 }
								 } }
	 			unset($_SESSION['tfees']);
            ...
            <div class="_25" style="float:right">
                ...
                <?php if($_GET['roll']){
					...
					//die();
					  ...
					  if(($class1['c_name']=="XI") || ($class1['c_name']=="XII" || $class1['c_name']=="XI STD") || ($class1['c_name']=="XII STD")){
						 $sid21 = $sid;
					  }else {
						  $sid21 = "0";
					  }								  
...
<input type="hidden" name="command" />
				...
				//$ssid=$_GET['ss_id'];
					$studentlist1=mysql_query("SELECT * FROM student WHERE ss_id=$ssid"); 
								  ...
								  $qry3=mysql_query("SELECT count FROM finvoice_no WHERE ay_id=$ay_id and category='$cat'"); 
								  ...
								  if($cat=="CL")
	   							  ...
	   							  if($cat=="CH")
	   	 							$cat="HSS";
								  $invoice_no=$cat.str_pad($row3['count'], 3, '0', STR_PAD_LEFT);
				...
				$bouncefind=mysql_query("SELECT * FROM finvoice WHERE ss_id=$ssid AND ay_id=$acyear AND c_status=1 AND service_charge_status IS NULL");
...
$st=mysql_query("select * from discount where ss_id=$ssid AND status=0");
...
<input type="hidden" name="discount_remark" value="<?= $remark?>">
    ...
    <span>
        ...
        <?php if($fdiscount){
        	$percent=($fdiscount/$feesrate)*100;
        ?>
        <div class="alert warning">Now Rs.<?php echo $fdiscount.' ('.round($percent,2).'% )';?>/- Discount applied</div>
        <?php } ?>
        <table>
		      <thead>
		        <tr>
                ...
                <?php } ?>
					...
					?>
						...
						if($B_there['c_status']==1){ ?>
							...
							<input id="chequecharge" class="biginput txt text" type="text" autocomplete="off" name="cheque_service"></td>
							<td></td>
						</tr>
						<?php } else { 
