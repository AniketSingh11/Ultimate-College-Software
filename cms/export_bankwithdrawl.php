...
$baid=$_GET['baid'];
$querylist ="SELECT * FROM bank_withdrawl";
if($baid){
...
$reader->setReadDataOnly(false);
     ...
     /*   for ($i = 1; $i < mysql_num_fields($result); $i++)
       ...
       $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "withdrawled By");$column++;
       ...
       $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Amount");$column++;
        ...
        
        {
           ...
           $name=$row['name'];
           $b_name=$row['b_name'];
           $withdrawl_by=$row['withdrawl_by'];
		   $type=$row['type'];
          $amount=number_format($row['amount'],2);
		  if($type=='1'){
			  $typename="Expence paid Cheque";
		  }else{
			  $typename="Self Cheque";
		  }
           ...
           $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $date);  $column++;
            ...
            $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $b_name); $column++;
             $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $withdrawl_by); $column++;
        ...
        $objPHPExcel->getActiveSheet()->setTitle("Bankwithdrawl Account Report");
        ...
        header('Content-Disposition: attachment;filename="BankwithdrawlAccount_report-list.xls"');
