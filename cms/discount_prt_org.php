...
<style type="text/css">
  ...
  .block-content-invoice1{
	...
	.total{ float:right; margin-right:50px;}
                        ...
                        <p><span style="position:absolute; left:100px"><?php echo $row['c_name']."-".$row1['s_name'];?></span><span style="position:absolute; left:690px"> <?php echo $invoice['day']."/".$invoice['month']."/".$invoice['year'];?></span></p>
				...
				<table class="table table-striped" id="table-example"cellpadding="0" cellspacing="0" style="clear:none;text-align:left; width:100%; margin-top:40px;">	
                	<tbody>
                    	...
                    	<tr>
							<td colspan="5"><center><?= $invoice['remark']?></center></td>
							<td class="total"><?php echo number_format($invoice['total'],2);?></td>
                        </tr>
                        
    
                    	<tr class="total_bar">
							<td class="grand_total1" colspan="4" width="220px" style=" font-size:11px !important;"><center>
 <?php $amount=number_format($invoice['total'],2);
						if(floor($amount)==$amount){
							$amount1=floor($amount);
							 echo convert_number_to_words($invoice['total']);
						}else{
						 echo convert_number_to_words($invoice['total']); }?> Rupees Only
</center></td>
							<td class="grand_total1" width="120px" style=" text-align:right">Total:</td>
							<td class="grand_total1 total">Rs. <?php echo number_format($invoice['total'],2);?></td>
						</tr>
                    ...
                    <td colspan="6">
                    <div>
                  <span style="float:left; margin-left:200px;top:180px; position:absolute; font-size:11px !important;"></strong></span>
                  </div>
                  </td>
                    </tr>
                    </tfoot>
