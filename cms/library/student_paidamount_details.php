 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1>Student Paid Fine Amount Book Report </h1>
		 </div>  <!-- #content-header -->	
...
<?php   if(isset($_GET['bid']))
            ...
            ?>
                ...
                <label for="select">Board :</label>
					 ...
					 <h3>						 
						Student  Paid Fine Amount Book Report
					 </h3>			 
				 </div>  <!-- /.portlet-header -->			
...
<div class="col-sm-12">
				            	...
				            	<div class="form-group">
									...
									<select id="student" name="student" onchange="select_employee()" data-required="true" class="form-control" style="width:50%">
                                        ...
                                        <?php 
										$emp_query="select * from student where b_id='$bid' and ay_id='$acyear' AND user_status='1' order by firstname asc";
			...
			?>
				...
				<div class="form-group">
								...
								<div class="form-group">
									<label for="name">Standard & Section : &nbsp;&nbsp;&nbsp;&nbsp;</label>
...
<?php if($s_id){ ?>  
        <center>      <a title="view" href="#info-dialog_<?php echo $s_id;?>" data-toggle="modal">    <button type="button" class="btn btn-warning">   Student profile</button></a></center>
                             ...
                             <table 
								...
								data-paginate="true">
									<thead>
										<tr>
                                            ...
                                            <!--  <th data-filterable="true" data-sortable="true">From Date</th>	--> 
                                             <th data-filterable="true" data-sortable="true">Fine Amount</th>	
									...
									<tbody>
										<?php	
								 	$emp_query="select * from lms_student_borrowbook  where ay_id='$acyear' and student_id='$s_id' and b_id='$bid' and status='1' and fine_amount!='0' and paid_status='1' ";
		...
		while($emp_display=mysql_fetch_array($emp_result))
		{
			...
			$fine_amount=$emp_display["fine_amount"];
			$query=mysql_query("SELECT * FROM board where b_id='$board'");
			$res=mysql_fetch_array($query);
			$board_name=$res["b_name"];
			
			$paid_status=$emp_display["paid_status"];
			if($paid_status==1){
			
			    $amount_status="<button type='button' class='btn btn-sm btn-success'>Paid</button>";
			}else{
			     
			    $amount_status="<button type='button' class='btn btn-sm btn-primary'>Pending</button>";
			}
		...
		?>                             
										 <tr>
                                            ...
                                            <td><center><?php echo $fine_amount; ?></center> </td>
                                            
                                             <!-- amount status -->  <td><?php echo $amount_status; ?></td>
                                             <td><button type="button" class="btn btn-sm btn-primary">Losted</button></td>	
                                            
                                             	<?php } else { ?>   <td><center><?php echo $fine_amount; ?></center> </td> 
                                                 <!-- amount status -->   <td><?php echo $amount_status; ?></td>	
                                              <td><button type="button" class="btn btn-sm btn-success">Closed</button></td>	<?php }?>		 
                                             
                                             								 
											 <td><a title="view" href="#styledModal<?php echo $emp_count.''.$sb_id;?>" data-toggle="modal"><img src="img/layout/view.png"/></a> 
											<!-- <a title="edit" href="borrow_bookdetails_edit.php?id=<?php echo $sb_id; ?>"><img src="img/layout/edit.png"/></a> 
...
<?php         
 ...
 function change_function1(){ 
	    ...
	    var student = parseFloat(document.getElementById('student').value);
		 ...
		 window.location.href = "student_paidamount_details.php?sid="+student+'&bid='+bid;
 ...
 function select_employee() { 
     ...
     var bid=document.getElementById('bid').value;
	  ...
	  if(student>0){
		  window.location="student_paidamount_details.php?sid="+student+'&bid='+bid;
	  }	
}
