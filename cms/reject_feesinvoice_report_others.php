    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li> Reject Fees Invoice Report</li>
			</ul>
		...
		$boardlist=mysql_query("SELECT * FROM board WHERE b_id=$bid"); 
		  ...
		  $enddate= $edate_year.$edate_month.$edate_day;
					...
					if($sdate && $edate && $bid){ 
						...
						$qry1=mysql_query("SELECT * FROM finvoice_others WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate.  "' AND ay_id='" . $acyear. "' and bid=$bid AND ay_id=$acyear  AND i_status='1' ");
			  ...
			  while($row1=mysql_fetch_array($qry1))
                ...
                <h1>Reject Fees Invoice Report</h1>
                <span style="">Board Name : <strong><?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?></strong></span>
                <span style="margin-left:20px;"><a href="reject_feesinvoice_download_others.php?sdate=<?php echo $sdate;?>&edate=<?php echo $edate;?>&b_id=<?php echo $bid;?>&ayid=<?php echo $acyear;?>" style="width:100px"><button class="btn btn-small btn-green"><img src="img/icons/packs/fugue/16x16/inbox-download.png"> Download Report</button></a></span>
			
				 
                <br><br>
				<div class="block-border">
					<div class="block-header">
                    	<h1>Reject Fees Invoice Report (<?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?>)</h1>                       
                        <span></span>
					</div>
					<div class="block-content">
						<table id="table-example" class="table">
							...
							<tbody>
                            <?php 
						$qry=mysql_query("SELECT * FROM finvoice_others WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate.  "' AND ay_id='" . $acyear. "' and bid=$bid AND ay_id=$acyear  AND i_status='1' ");
...
//echo "SELECT * FROM class WHERE c_id=$cid";		die;		
				...
				?>
								...
								<td class="view"><center><a href="feesinvoice_reject_detail_others.php?fiid=<?php echo $row['fi_id'];?>&bid=<?php echo $bid;?>"><button class="btn btn-primary btn-small"><img src="img/icons/packs/fugue/16x16/pictures-stack.png"> View</button></a></center></td>
