...
if (isset($_POST['place-order']))
{
	  ...
	  $sql="INSERT INTO mfinvoice (fr_no,fi_name,fi_total,fi_ptype,fi_day,fi_month,fi_year,ss_id,c_id,s_id,category,stype,fi_by,bid,ay_id,pay_number,get_amount,balance,poor_student_pay,fund_amount,funds,fi_by_name) VALUES
('$invoice_no','$ss_name','$total','$ptype','$day','$month','$year','$ssid1','$cid1','$sid1','$category','$stype','$user','$bid1','$acyear','$pay_number','$get_amount','$balance','$poor_pay_amount','$fund_amount','$fund','$user')";
//die();
		...
		<div id="main-content">
			...
			if(empty($_SESSION['fees'])){
				...
				/****************************************** Pending Amount Start **********************************/
					$totalpending=0;
					if($lacyear){
									echo $lstudentlist="SELECT * FROM student WHERE admission_number LIKE '$rollno' AND ay_id=$lacyear"; 
				...
				$sql1=mysql_query("SELECT * FROM mfrate WHERE c_id=$lcid AND b_id=$bid AND ay_id=$lacyear AND rate='$ls_type' AND s_id=$lsid21 ORDER BY fgd_id");
					 ...
					 $f_to12=0;
										if(!empty($frid)) { 
					 $qry3="SELECT * FROM mfinvoice WHERE bid= '" . $bid. "' AND ay_id='" . $lacyear. "' AND c_id = '" . $lcid. "' AND ss_id = '" . $lssid. "' AND c_status!='1'";							
							//echo $qry1;
