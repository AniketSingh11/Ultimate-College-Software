    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover"><?php if($cid){ echo $class['c_name']." - "; }?>Pre Admission selection</li>                
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1><?php if($cid){ echo $class['c_name']." - "; }?>Pre Admission List</h1>
                <?php if($cid){?><a href="pre_admission_mark.php?bid=<?php echo $bid."&cid=".$cid; ?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-success" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> Student Mark Update</button></a> <?php }?>
                <a href="pre_admission_single.php" title="add"  style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">Pre Admission Form</button></a>
                <a href="pre_admission_mail.php" title="add"  style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">Selected Student Mail</button></a>
            ...
            <div class="grid_12">
				<div class="block-border" id="tab-panel-2">
					<div class="block-header">
						<h1><?php if($cid){ echo $class['c_name']." - "; }?>Pre Admission selection</h1>
                        <ul class="tabs">
							<li><a href="#tab-lorem">Student List</a></li>
							<li><a href="#tab-dolor">Selected Student List</a></li>
						</ul>
                        <div class="_25" style="margin:8px 0 0 20px">
                            <select name="down" id="down" onchange="download_function()">
                            	<option value=''>Download Report</option>
                                <option value='1'>All Student List</option>
                                <option value='2'>Process Student List</option>
                                <option value='3'>Selected Student List</option>
                                <option value='4'>Rejected Student List</option>
                            </select>
                        </div>
                        <div class="_25" style="margin:8px 0 0 20px">
                            <select name="gen" id="gen" onchange="download_function1()">
                            	<option value='all'>All</option>
                                <option value='M' <?php if($gen=="M"){ echo "selected";}?>>Alb to Male - Female</option>
                                <option value='F' <?php if($gen=="F"){ echo "selected";}?>>Alb to Female - Male</option>
                            </select>
                        </div>
					</div>
					  ...
					  while($row=mysql_fetch_array($qryselect))
						{
							...
							$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid1"); 
                                ...
                                <button class="btn btn-small btn-success" >Selected </button><?php } else if($row['status']=='2'){?><button class="btn btn-small btn-error" >Rejected </button> <?php } else { ?><button class="btn btn-small btn-warning" >Processing </button><?php } ?>
                                 ...
                                 <a href="pre_admission_mark_edit.php?paid=<?php echo $row['pa_id'];?>&bid=<?php echo $bid; if($cid){ echo "&cid=".$cid;}?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="pre_admission_select_delete.php?paid=<?php echo $row['pa_id'];?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
			...
			<div id="info-dialog<?php echo $count;?>" title="<?php echo $row['pa_admission_no']; ?>, This student details" style="display: none;">
                ...
                <p>Status : <?php if($row['status']=='1'){ ?>
						...
						<div id="tab-dolor" class="tab-content">
							...
							<tbody>
                            <?php 
							$qry="SELECT * FROM pre_admission WHERE status = '1' AND ay_id='$acyear'";							
							if($bid){
								$qry .=" AND b_id='$bid'";
							}
							if($cid){
			  ...
			  while($row=mysql_fetch_array($qryselect))
        		{
					$cid1=$row['c_id'];
							$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid1"); 
                                ...
                                <td><center><?php echo $row['phone_number']; ?></center></td>
                                <td><center><?php echo $class['c_name']; ?></center></td>
                                <td><center><?php echo $row['mark']; ?></center></td>
                                 <td width="100px;">
                                 <a href="javascript:void(0);" onclick="$('#sinfo-dialog<?php echo $count;?>').dialog({ modal: true });" title="Details"><img src="./img/detail.png" alt="edit"></a>
                                  <a href="pre_admission_mark_edit.php?paid=<?php echo $row['pa_id'];?>&bid=<?php echo $bid; if($cid){ echo "&cid=".$cid;}?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="pre_admission_select_delete.php?paid=<?php echo $row['pa_id'];?>&bid=<?php echo $bid; if($cid){ echo "&cid=".$cid;}?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
                                 <a href="pre_admission_prt.php?paid=<?php echo $row['pa_id'];?>" title="print" target="_blank"><img src="./img/print.png" alt="print"></a>
                                 </td>
								</tr> 
                                                     <!-- Modal Box Content -->
			<div id="sinfo-dialog<?php echo $count;?>" title="<?php echo $row['pa_admission_no']; ?>, This student details" style="display: none;">
           	<p>Admin NO : <strong><?php echo $row['pa_admission_no']; ?></strong></p>
                ...
                <p>Mark : <strong><?php echo $row['mark']; ?></strong> </p> 
...
function change_function() { 
	...
	function download_function() { 			
	 ...
	 if(down){
	 window.open('pre_admission_down.php?down='+down+'<?php echo "&bid=".$bid."&cid=".$cid."&acid=".$acyear."&gen=".$gen;?>','_blank');	  
	 }
	}
	function download_function1() { 			
     var down =document.getElementById('gen').value;
	 if(down){
	window.location.href ='pre_admission_select.php?gen='+down+'<?php echo "&bid=".$bid."&cid=".$cid;?>';	  
	 }
	}
	</script>
