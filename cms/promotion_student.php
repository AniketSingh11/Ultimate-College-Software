 ...
 if (isset($_POST['submit']))
{
    ...
    $old_cid=mysql_real_escape_string($_POST['o_cid']);
    $classfind=mysql_query("select * from class where c_id=$old_cid");
    
    $cname=mysql_fetch_array($classfind)['c_name'];
    ...
    { 
					 ...
					 if(mysql_num_rows($query1)=="0"){
					 	if($cname=="UKG"){
					 		$admincount=mysql_query("select * from admin_no_count where name='school admission'");
					 		$no=mysql_fetch_array($admincount)['count'];
					 		$row['admission_number']=$no;
					 		$no=$no+1;
					 		$sql="update admin_no_count set count=$no where name='school admission'";
					 		$result = mysql_query($sql) or die("Could not Update data into DB: " . mysql_error());
					 	}
