    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
              ...
              <li> Cheque Fees Invoice Report</li>
			</ul>
...
//echo"SELECT * FROM board WHERE b_id=$bid";die;		
		  ...
		  $enddate= $edate_year.$edate_month.$edate_day;
					...
					$qry1=mysql_query("SELECT * FROM finvoice_others WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate.  "' AND ay_id='" . $acyear. "' and fi_ptype='cheque' AND bid=$bid AND ay_id=$acyear  AND i_status='0' ORDER BY fr_no DESC ");
			  ...
			  while($row1=mysql_fetch_array($qry1))
                ...
                <h1>Cheque Fees Invoice Report</h1>
                <span style="">Board Name : <strong><?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?></strong></span>
                <span style="margin-left:20px;"><a href="cheque_feesinvoice_download_report_others.php?sdate=<?php echo $sdate;?>&edate=<?php echo $edate;?>&b_id=<?php echo $bid;?>&ayid=<?php echo $acyear;?>" style="width:100px"><button class="btn btn-small btn-green"><img src="img/icons/packs/fugue/16x16/inbox-download.png"> Download Report</button></a></span>
			
				 
                <br><br>
				<div class="block-border">
					<div class="block-header">
                    	<h1>Cheque Fees Invoice Report (<?php if($bid=='All'){ echo "All";}else{ echo $board['b_name']; }?>)</h1>                       
                        <span></span>
					</div>
					<div class="block-content">
						<table id="table-example" class="table">
							...
							<tbody>
                            <?php 
						$qry=mysql_query("SELECT * FROM finvoice_others WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate.  "' AND ay_id='" . $acyear. "' and  fi_ptype='cheque' AND bid=$bid AND ay_id=$acyear  AND i_status='0'  ORDER BY fr_no DESC");
						//echo"SELECT * FROM finvoice WHERE (fi_year*10000) + (fi_month*100) + fi_day between '" . $startdate. "' AND '" . $enddate.  "' AND ay_id='" . $acyear. "' and  fi_ptype='cheque' AND bid=$bid AND ay_id=$acyear  AND i_status='0'  ORDER BY fr_no DESC";die;
							...
							//$qry=mysql_query($qry);
							//$qry=mysql_query("SELECT c_id,s_id,fr_no,fi_name,fi_day,fi_month,fi_year,stype,fi_by,fi_total,fi_id,ss_id FROM finvoice $count=1;
			  while($row=mysql_fetch_array($qry))
        		{ 
				$ssid=$row['ss_id'];
				$cid=$row['c_id'];
				...
				?>
								...
								<td class="view"><center><a href="chequefeesinvoice_detail_others.php?fiid=<?php echo $row['fi_id'];?>&bid=<?php echo $bid;?>"><button class="btn btn-primary btn-small"><img src="img/icons/packs/fugue/16x16/pictures-stack.png"> Edit</button></a></center></td>
								 <?php 
							$count++;
							} ?>                                  																
							</tbody>
