	...
	if(!$bid){
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
				...
				<li class="no-hover">Subject Management</li>                
			</ul>
        ...
        <a href="board_select1.php" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a>
			...
			<?php
				...
				if($cid && $sid){
							...
							$sectionlist=mysql_query("SELECT * FROM section WHERE s_id=$sid"); 
								  ...
								  ?>
			<div class="grid_12">
				<h1><?php echo $class['c_name']."-".$section['s_name'];?> Subject List</h1>
                <a href="subjectmng_new.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-success" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> Import Subject Data</button></a>
                <a href="subjectmng_single.php?cid=<?php echo $_GET['cid']; ?>&sid=<?php echo $_GET['sid']; ?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">  Add Single</button></a>
				<?php $msg=$_GET['msg'];
				if($msg=="dsucc"){?>
			  ...
			  while($row=mysql_fetch_array($qry))
        		{
					...
					?>
								...
								<td width="150px"><center><?php echo $row['s_code'];?></center></td>
                                <td><center><?php echo $row['s_name']; if($paper==1){ echo " (Two Paper) ";}?></center></td>
                                <td><center><?php if($stype==0){ echo " FA-SA Concept ";}else{ echo "100 Marks";}?></center></td>
                                 <td width="80px;">
                                 ...
                                 <a href="subjectmng_edit.php?subid=<?php echo $row['sl_id'];?>&cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&bid=<?php echo $bid;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="subjectmng_delete.php?subid=<?php echo $row['sl_id']; ?>&cid=<?php echo $cid; ?>&sid=<?php echo $sid;?>&bid=<?php echo $bid;?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
                                 </td>
								</tr> 
                                                     <!-- Modal Box Content -->
			<div id="info-dialog<?php echo $count;?>" title="<?php echo $row['s_name']; ?>, This Subject details" style="display: none;">
                    ...
                    <p>Subject Code: <strong><?php echo $row['s_code']; ?></strong></p>
                 	<p>Subject Name : <strong><?php echo $row['s_name']; if($paper==1){ echo " (Two Paper) ";}?></strong></p> 
...
// Code that uses other library's $ can follow here.
  ...
  <script type="text/javascript">
	...
	function change_function() { 
	 ...
	 window.location.href = 'subject_mng.php?bid='+cid;	  
	} 
  </script>
  <!-- end scripts-->
