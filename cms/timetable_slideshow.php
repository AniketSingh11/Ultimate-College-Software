			  ...
			  while($topsubject1=mysql_fetch_array($topsubjectlist1))
                ...
                <div align="center" id="printablediv" >  <!-- ImageReady Slices (Bonafide(1).jpg) -->
					...
					$timelist1=mysql_query("SELECT * FROM timetable WHERE c_id=$cid1 AND s_id=$sid1 AND b_id=$bid AND ay_id=$acyear"); 
					  ...
					  ?>
						<table id="table-example" class="table1" align="center" style="background:#FFFFFF;">
							<thead>
								<tr>
                                	<th>Day</th>
                                    <?php
                                ...
                                <tr class="gradeC">
								   ...
								   $slist=mysql_fetch_array($subjectlist1);
                                    ...
                                    <?php
									 $subid=$peroid[$i];
									//$p1=$timetable['p1'];
									 $qry1=mysql_query("SELECT * FROM subject WHERE c_id=$cid1 AND s_id=$sid1 AND ay_id=$acyear");							
			  while($row1=mysql_fetch_array($qry1))
        		{ $slid=$row1['sl_id'];
								   $subjectlist1=mysql_query("SELECT * FROM subjectlist WHERE sl_id='$slid'"); 
								   $slist=mysql_fetch_array($subjectlist1);
				if($subid==$row1['sub_id']){?>
									<?php echo $slist['s_name'];?>
                                    <?php } }
									 $qry2=mysql_query("SELECT * FROM extraperoid WHERE ay_id=$acyear");							
			  while($row2=mysql_fetch_array($qry2))
        		{ 
				if($subid==$row2['ep_code']){?>
                                    <?php echo $row2['ep_name'];?>
                                    <?php } } ?>
								<!--<br><br>
