...
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Paid Amount");$column++;
			  ...
			  while($row1=mysql_fetch_array($qry1))
        		{
					...
					$paid=paid_amount($bid,$cid,$sid,$acyear,$ss_id);
							...
							if($fullpay && $pendpay==0 && $ptype1=="All"){ 
							    ...
							    $objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $discount1['fdis_name']); $column++;
								...
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $totalamount); $column++;
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $total); $column++;
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $tot); $column++;
							    ...
							    $rowCount++;
										...
										$content .= stripslashes($discount1['fdis_name']). ',';
										$content .= stripslashes($student['stype']). ',';
										$content .= stripslashes($ptypename). ',';
										$content .= stripslashes($totalamount). ',';
										$content .= stripslashes($total). ',';
										$content .= stripslashes($tot). ',';
										$content .= "\n";
							$count++;	
							}
...
header("Content-Disposition: attachment;filename=MonthlyFees_Paid_report-list($ptypename$class_name-$section_name).xls");
