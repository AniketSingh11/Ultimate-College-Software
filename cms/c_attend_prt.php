...
<div style="float:left; width:50%; text-align:left; margin-left:5%; font-size:21px;"><?php echo "SPMS/AC/".$acyear_name."/".$row['ref_number']; ?></div>
<div style="float:left; width:40%; text-align:right; margin-right:3%; font-size:21px;">Date: <?=$row['c_date']?></div>
</div>
 
<h2 style="line-height:46px; text-align:center;width:100%; font-family:Impact, Haettenschweiler, 'Franklin Gothic Bold', 'Arial Black', sans-serif; font-size:30px;">ATTENDANCE CERTIFICATE</h2>
<div style="margin-top:85px;width:866px;margin-left:4px; line-height:60px; text-align:justify;">
<font color="#2E2F65" style="font-size:28px;">This is to certify that <b><?php echo $row['name']." (".$row['admin_no'].")"; ?> </b>was a student of this institution studying in standard <b> <?php echo $row['standard']; ?></b> during the school year <b><?php echo $row['year']; ?></b> and the no of days of his/her attendance was<b> <?php echo $row['percent']; ?></b> out of <b><?php echo $row['work']; ?></b> working days.. </font></div>
...
$().ready(function() {
	...
	$("#show").change(function(){
		...
		window.location.href='c_attend_prt.php?cattid=<?=$cattid?>&show=hide';
		}else{
			window.location.href='c_attend_prt.php?cattid=<?=$cattid?>&show=show';
		}
