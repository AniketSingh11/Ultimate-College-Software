    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Bank Deposit Details</li>
			</ul>
            ...
            <?php 
					...
					if($baid){
							 $classlist=mysql_query("SELECT * FROM bank_account WHERE ba_id=$baid"); 
								  ...
								  ?>
			<div class="grid_12">
				<h1><?php if($baid){ echo $class['name']."-".$class['account_no'];} else{ echo "All Account";} ?>  Bank Deposit Details</h1>
			  ...
			  while($row1=mysql_fetch_array($qry1))
                ...
                <label for="select">Account Detail :</label>
                 ...
                 <span style="margin:0px 50px 0 0px; float:right;"><img src="img/icons/packs/fugue/16x16/inbox-download.png"> Total : <strong>Rs. <?php echo number_format($total,2); ?></strong><?php  if($baid){?> &nbsp; | &nbsp; Account Cash :<button class="btn btn-small btn-success"><?=$class['amount']?> Rs/-</button><?php } ?></span> 
                 <?php $msg=$_GET['msg'];
				if($msg=="dsucc"){?>
			  ...
			  while($row=mysql_fetch_array($qry))
        		{
					$baid1=$row['ba_id'];
                                ...
                                <td><center>Rs. <?php echo number_format($row['amount'],2); ?></center></td>                               
                                 ...
                                 <?php if($row['type']=='0'){?>
                                 <a href="bdeposit_mng_edit.php?bcid=<?php echo $row['bc_id'];?>&baid=<?php echo $baid;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a> <a href="bdeposit_mng_delete.php?bcid=<?php echo $row['bc_id']; ?>&baid=<?php echo $baid; ?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
                                 <?php } ?></td>
								</tr> 
                                <div id="info-dialog<?php echo $count;?>" title="<?php echo $row['b_name']."-".$row['account_no']; ?> Bank Deposit Details" style="display: none;">
...
// Code that uses other library's $ can follow here.
  ...
  <script type="text/javascript">
	$().ready(function() {
	 ...
	 window.location.href = 'bdeposit_mng.php?baid='+cid;	  
	} 
  </script>
  <!-- end scripts-->
