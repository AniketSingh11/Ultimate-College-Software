 ...
 if (isset($_POST['submit']))
{
	...
	if ($date2<$date1) {
	header("location:sleave_edit.php?slid=$slid&msg=errdate");
	}else if($date2==$date1){
		$days=1;
		}
	
	$l_query="UPDATE staff_leave SET l_type='$l_l_type',l_a_date='$l_l_a_date',l_from='$l_l_from',l_to='$l_l_to',l_total='$days',l_des='$l_descrip',status='1',f_month='$ldate_month',f_year='$ldate_year' WHERE id='$slid'";
	
	$l_result=mysql_query($l_query);	
	if($l_result)	
	{
		header("location:sleave_edit.php?slid=$slid&msg=succ");
	}		
	else
	{
		header("location:sleave_edit.php?slid=$slid&msg=err");
	}
}
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Edit Leave</li>
			</ul>
            ...
            <?php } ?>
				...
				<div class="block-border">
					<div class="block-header">
						<h1>Edit Leave</h1><span></span>
					</div>
                    <?php 
					$slid=$_GET['slid'];
							$stafflist=mysql_query("SELECT * FROM staff_leave WHERE id=$stid"); 
					...
					<form id="validate-form" class="block-content form" action="" method="post">
							...
							<p>
								<label for="select">Leave Types : <span class="error">*</span></label>
									...
									$ltype=$staff['l_type'];
											...
											if($ltype==$student12['lt_name']){
											?>                                            
                                                <option value='<?php echo $student12['lt_name']?>' selected><?php echo $student12['lt_name']." ".$student12['middlename']." ".$student12['lastname']; ?></option>
                                                <?php }else{?>
                        ...
                        <div class="_50">
							<p>
                                ...
                                <input id="startdate" name="l_from" class="required" class="startdate" type="text" value="<?php echo $staff['l_from'];?>"/>
                            </p>
						</div>
                        <div class="_50">
							<p>
                                <label for="textfield">End Date:<span class="error">*</span></label>
                                <input id="enddate" name="l_to" class="required" class="enddate" type="text" value="<?php echo $staff['l_to'];?>"/>
                            </p>
						</div>
                        <div class="_100">
							<p><label for="textarea">Description :</label>
                            <textarea name="des"  rows="5"><?php echo $staff['l_des'];?> </textarea>
						</div>
						...
						<div class="block-actions">
							...
							<ul class="actions-left">
                            <input type="hidden" class="medium" name="slid" value="<?php echo $_GET['slid'];?>" >
