 ...
 <!--! end of #search-bar -->
    ...
    <div id="main" role="main">
        ...
        if($_POST['submit']){
        	...
        	$amt=$_POST['amount'];
        	$status=0;
        	$up=mysql_query("update tc_xi_kg set status='0',tc_amount='$amt',date_day='$date_day',date_month='$date_month',date_year='$date_year' where id='$id'");
        	
        	if($up){
        		header('Location:tc_amount_kg.php?id='.$id.'&update=success');
        		exit;
        	} else {
        			echo 'fsd';
        			die;
		...
		<div id="main-content">
            ...
            <center><a href="tc_kg.php?tcid=<?php echo $_GET['id'];?>" target="_blank"><button class="btn btn-small btn-warning" ><img src="img/icons/packs/fugue/16x16/block--arrow.png"> Click Here To Print the Last Created Certificate</button></a></center>
            </div>
                <?php }?>
				...
				<form action="tc_amount_kg.php" method="post">
                ...
                <input type="submit" value="Submit" name="submit" class="btn btn-green" style="width:120px">
               	<?php }	else { ?>
               	<input type="button" value="Print" name="Print" onClick="window.open('tc_amount_prt_kg.php?id=<?= $ffi_id?>')"  class="btn btn-green" style="width:120px">
...
function redirect(){
	 document.location.href = "tc_xi_kg.php?bid=1";
}
</script>
  <!-- end scripts-->
