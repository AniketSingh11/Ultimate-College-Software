 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1>Edit Hostel Fees Details  <a href="hostel_fees_amount.php"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
...
<form id="validate-enhanced" action="" class="form parsley-form" method="post" enctype="multipart/form-data">
     ...
     <p class="title"> Academic Year :2014 - 2015 </p>     
       <p class="title"> Standard :<?=$c_name?> </p>
     ...
     <input type="hidden" name="fid" value="<?=$id?>"> 
        ...
        <?php $res=mysql_query("select * from hms_room_type where status='0'");
									...
									$hrt_id=stripslashes($row["hrt_id"]);
									$qry1=mysql_fetch_array(mysql_query("select * from hms_feestype where hfs_id='$id' and room_type='$hrt_id'"));
									$amount=$qry1["amount"];
									
									?>
								 
              <td><div class="form-group"><input type="text" style="width: 150px;"  name="amount<?=$hrt_id?>" value="<?=$amount?>" data-digit="true" class="form-control" data-required="true"></div></td>
              <?php }?>
