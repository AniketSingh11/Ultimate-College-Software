 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1>Overall Room Report <a  onclick="history.go(-1);"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
...
<form id="validate-enhanced" action="" class="form parsley-form" method="post" enctype="multipart/form-data" name="first">
				          ...
				          <div class="portlet-content">                      
                            	...
                            	<div class="form-group">
									...
									<select id="sel_book"  onchange="select_employee()"   placeholder="Please select Book" name="sel_book" data-required="true" class="form-control" style="width:70%">
                                      <option value="">Select</option>
                                        <?php 
										$emp_query="select * from hms_room where   status='0' order by category asc";
										...
										while($emp_display=mysql_fetch_array($emp_result))
										{
                                           ...
                                           $floor_name=$row["floor_name"];
											...
											?>
                                        <option value="<?php echo $hr_id;?>" <?php if($s_id==$hr_id){?>selected="selected" <?php }?>><?php echo  $cat_name." / ".$floor_name." / ".$room_number; ?></option>
                                  <?php } ?>								
                            		</select>
								</div>  
...
<?php if($s_id){ ?>  
                 <center><a href="export_hostel_report.php?type=hr_id&id=<?=$s_id?>"><button type="button" class="btn btn-warning">Download Excel</button></a></center>
                             ...
                             <table 
								...
								data-paginate="true">
									...
									<tbody>
										...
										$emp_query="select * from hms_student_room  where hr_id='$s_id' and status='0' order by category desc";
...
$emp_count++;
											...
											$emp_query="select * from hms_staff_room  where hr_id='$s_id' and status='0' order by join_date desc";
...
<?php         
 ...
 function change_function1(){ 
	    ...
	    var staff = parseFloat(document.getElementById('sel_book').value);
		 ...
		 window.location.href = "report_book_details.php?fid="+staff+'&bid='+bid;
 ...
 if(selbook>0){
		   window.location="report_room_details.php?rid="+selbook;
	  }	
}
