 ...
 include("header.php");
  ...
  if(isset($_POST["submit"]))
  {
     ...
     // $to_date=$to[2]."-".$to[1]."-".$to[0];
      ...
      $res1=mysql_query("select * from hms_student_room  where 	admission_number='$admission_number'  and status='0'") or die(mysql_error());
      $chk=0;
      while($row1=mysql_fetch_array($res1))
      {
         $chk=1; 
         $err_msg.="Student  Already Booked Room..  &nbsp;";
      ...
      {
        $query="insert into hms_student_room(category,floor,hr_id,hrc_id,admission_number,firstname,lastname,reg_class,r_ay_id,room_date,join_date,date)
	           values('$hostel_id','$floor','$room_id','$cart_id','$admission_number','$firstname','$lastname','$c_id','$acyear','$join_date','$join_date','$date')";
	    ...
	    $query="insert into hms_studentcash_amount(hsr_id,admission_number,amount,r_ay_id,date)
	    values('$sb_id','$admission_number','$amount','$acyear','$date')";
	    mysql_query($query);
	    ...
	    header("location: regroom_preferstudent_details.php?msg=succ");
      }else{
           
           header("location: regroom_preferstudent_details.php?msg=err&err_msg=$err_msg");
 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1> Apply Register Room For Prefer Hostel Student   <a onclick="history.go(-1)"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
 ...
 <div class="alert alert-success">
				...
				<a href="invoice_fees.php?id=<?=$_GET["ss_id"]?>&fees_type=Register Fees" class="btn btn-info" type="button" <button>Pay Amount</a> 
			</div>	
<?php } ?>
 <?php if($_GET["msg"] == 'err') { ?>
...
<?php   if(isset($_GET['cid']))
            ...
            ?>
                ...
                <label for="select">Hostel :</label>
				 ...
				 <h3>						 
						Apply Register Room  For Prefer Hostel Student
					 </h3>			
				 </div>  <!-- /.portlet-header -->			
...
<div class="col-sm-12">
            ...
            $section_name=$res["s_name"];
											...
											?>
                                        <option value="<?php echo $ss_id;?>"><?php echo  $emp_display["admission_number"]."-".$emp_display["firstname"]." ".$emp_display["lastname"]." - ".$class_name."-".$section_name; ?></option>
                                  <?php } } ?>								
                            		</select>
								</div>       
								...
								<div class="form-group" >
									<label for="name">Advance Amount</label>
									   ...
									   <input id="amount"  style="width:35%" name="amount" placeholder="Amount"  data-required="true" class="form-control">          
								</div>  
...
alert(v);
 ...
 function change_function1(){ 
	    ...
	    var cid=document.getElementById('cid').value;
		 window.location.href = 'regroom_preferstudent_details.php?cid='+cid;
