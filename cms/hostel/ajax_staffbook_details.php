...
$query="select *  from lms_book where avilable_qty!='0' and status='0' order by book_title asc";
$res=mysql_query($query) or die(mysql_error());
while($row=mysql_fetch_array($res))
{
    $book_id=$row["b_id"];
   
    $query1="select  * from `lms_staff_borrowbook` where book_id='$book_id' and staff_id='$staff_id' and status='0' limit 1";
    ...
    if($c=="0")
    {
        ?><option value="<?=$book_id?>"><?php echo $row["book_no"]."-".$row["book_title"];?></option>
                        <?php 
