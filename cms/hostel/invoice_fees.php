...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
     ...
     elseif($fees_type=="Changeroom Fees")  

       {
           
           $sql="INSERT INTO hms_hinvoice_sumarry (hin_id,hsr_id,hr_id,hrc_id,ay_id,fees_name,fees_type,amount,date) VALUES
           ('$lastid','$hsr_id','$hr_id','$hrc_id','$acyear','Changeroom Fees','Changeroom Fees','$total','$date')";
           
           
           
          $result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
          
          $qry1=mysql_query("UPDATE hms_student_changeroom SET payment_status='1' WHERE hsr_id='$hsr_id'  and hr_id='$hr_id' and hrc_id='$hrc_id' and ay_id='$acyear'");
...
<div class="col-sm-12">
				            	 ...
				            	 <div class="form-group">
									...
									<select id="student" onchange="select_employee()"  name="student" data-required="true" class="form-control">
										...
										while($emp_display=mysql_fetch_array($emp_result))
										{
											...
											$ss_id=$row1["ss_id"];
											?>
                                        <option value="<?php echo $ss_id;?>" <?php if($s_id==$ss_id){ echo "selected"; }?>><?php echo  $row1["admission_number"]."-".$row1["firstname"]." ".$row1["lastname"]; ?></option>
                                  <?php } ?>								
                            		</select>
								</div>      
     ...
     <div class="col-sm-6">
		...
		$emp_count=1;
		 $emp_display=mysql_fetch_array($emp_result);
			...
			$qry1=mysql_query("select * from hms_hinvoice where hsr_id='$hsr_id' and fees_type='Register Fees'");
		
			if(mysql_num_rows($qry1)==0)
			{
			    $checkinvoice=1;
			    $fees_type="Register Fees";
        ...
        </tr>	<input type='hidden' name='fees_type' value='<?=$fees_type?>'> 
			    <?php
			}else{
			
			$qry2=mysql_query("select * from hms_hinvoice where hsr_id='$hsr_id' and ay_id='$acyear'");
			
			if(mysql_num_rows($qry2)==0)
			{
			    $fees_type="Renew Fees";
			    $checkinvoice=1;
			    $res=mysql_query("select * from hms_fees_structure where section='$c_id' and ay_id='$acyear'");
			    $row=mysql_fetch_array($res);
			    $hfs_id=$row["hfs_id"];
        ...
        <?php
			...
			$qry=mysql_query("select * from hms_student_changeroom where hsr_id='$hsr_id' and ay_id='$acyear' and payment_status='0'");
			...
			$amount=$res["amount"];
			$total=$amount;
			$fees_type="Changeroom Fees";
			?>
						    		    
			        <tr id="sb_id<?=$counts?>" bgcolor="#FFFFFF" >
			                    <td>1</td>
			                    
			                    <td>Change Room  Fees</td>
			                    <td class="total"><b>Rs.<?php echo  number_format($amount,2);?></b></td>
...
?>
		 ...
		 <?php if($counts==0){ //echo "<br><h1 style='color:red';>No Books Available </h1>";
		   ...
		   echo "<br><h1 style='color:red';>No Fees Available </h1>";
...

{
 ...
 function select_employee() { 
	  ...
	  if(student>0){
		  window.location="invoice_fees.php?id="+student;
	  }	
}
