 ...
 <div id="wrapper">
     ...
     <div id="content">	
		 <div id="content-header">
			 <h1>Overall floor Report <a  onclick="history.go(-1);"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
                            	...
                            	<div class="form-group">
									...
									<select id="sel_book" onchange="select_employee()"  placeholder="Please select Book" name="sel_book" data-required="true" class="form-control" style="width:50%">
                                      <option value="">Select</option>
                                        <?php 
										$emp_query="select * from hms_floor where   status='0' order by category asc";
										...
										while($emp_display=mysql_fetch_array($emp_result))
										{
                                           ...
                                           $cat_name=$row["h_name"];
											...
											?>
                                        <option value="<?php echo $hf_id;?>" <?php if($s_id==$hf_id){?>selected="selected" <?php }?>><?php echo  $cat_name." / ".$emp_display["floor_name"]; ?></option>
                                  <?php } ?>								
                            		</select>
								</div>  
...
<?php if($s_id){ ?>  
                 <center><a href="export_hostel_report.php?type=floor&id=<?=$s_id?>"><button type="button" class="btn btn-warning">Download Excel</button></a></center>
                             ...
                             <table 
								...
								data-paginate="true">
									...
									<tbody>
										...
										$emp_query="select * from hms_student_room  where floor='$s_id' and status='0' order by category desc";
...
$emp_count++;
											...
											$emp_query="select * from hms_staff_room  where floor='$s_id' and status='0' order by join_date desc";
...
<?php         
 ...
 if(selbook>0){
		   window.location="report_floor_details.php?fid="+selbook;
	  }	
}
