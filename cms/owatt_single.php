 ...
 if (isset($_POST['submit']))
{
	...
	$select_record=mysql_query("SELECT * FROM others WHERE status='1' order by fname asc");
					...
					{ 
						...
						$select_record1=mysql_query("SELECT satt_id FROM sattendance where (day='$date_day') AND m_id=$mid AND o_id=$o_id AND ay_id=$acyear");
						$queryfetch1=mysql_fetch_array($select_record1);
						
						if($queryfetch1){
							header("Location:owatt_single.php?mid=$mid&msg=already");
						}else{
							...
							$apply1=mysql_insert_id();
							 ...
							 $qry=mysql_query("INSERT INTO `sattendance` (`day`,`month`,`year`,`result`,`m_id`,`o_id`,`lt_id`,`reason`,`ay_id`,`result_half`,`l_apply`) VALUES ('$date_day', '$date_month','$date_year', '$postTest','$mid','$o_id','$ltid','$reason','$acyear','$resulthalf','$apply1')");
...
{
 header("Location:owatt_single.php?mid=$mid&msg=succ");
}
}
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
				...
				<li class="no-hover"><a href="owatt_mng.php?mid=<?php echo $mid;?>" title="Result Management">Other Staff Attendance managemrnt</a></li>
                <li class="no-hover">Add Other Staff Attendance</li>
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1>Add Other Staff Attendance <b>( <?php echo $month['m_name'];?> )</b></h1>                
            ...
            <?php } ?>
				<div class="block-border">
					<div class="block-header">
						<h1>Add Other Staff Attendance <b>( <?php echo $month['m_name'];?> )</b></h1><span></span>
					</div>
...
<script defer src="js/zebra_datepicker.js"></script>
  <!-- end scripts-->
   ...
   $().ready(function() {
        ...
        $.get("owattenance.php",{value:value},function(data){
			$( "#test" ).html(data);
        });
