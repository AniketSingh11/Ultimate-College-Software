   ...
   $sdate_split1= explode('/', $value);		 
		  ...
		  $finaldate=$sdate_year."-".$sdate_month."-".$sdate_day;		  
 		  echo '<div class="_100">
					...
					{ 
					$st_id=$student12['st_id'];
					
					$emp_query="select id,l_type,l_des,h_type,from_date,to_date from staff_leave where (from_date<='$finaldate' AND to_date>='$finaldate') AND st_id=$st_id AND status='1'";
					...
					//echo $morning."-".$evening."-".$fullday."<br>";
                    echo '<tr>
                        <td><center>'.$att.'</center></td>
                        <td><center>'.$student12['staff_id'].'</center></td>
                        <td><center>'.$student12['fname']." ".$student12['lname'].'</center></td>
                        <td><center><input type="radio" id="name"  name="'.$student12['st_id'].'" class="text err" value="1"';
						...
						echo 'disabled';}
						echo '/></center></td>
                        <td><center><input type="radio" id="name" name="'.$student12['st_id'].'" class="text err" value="0"';
						...
						echo 'disabled';}
						echo '/></center></td>
                        <td><input type="radio" id="name" name="'.$student12['st_id'].'" class="text err" value="off-M"';
						if($morning){
							$leave="off-M";
						echo 'checked="checked"';}
						if($emp_display){
						echo 'disabled';}
						echo '/>Morning<br>
                            <input type="radio" id="name" name="'.$student12['st_id'].'" class="text err" value="off-E"';
						if($evening){
							$leave="off-E";
						echo 'checked="checked"';}
						if($emp_display){
						...
						echo '/>Afternoon';
							if($emp_display){
								echo '<input type="hidden" id="ltid" name="'.$student12['st_id'].'" class="text err" value="'.$leave.'" />';
							}
                        echo '</td>
                        <td>';
								...
								echo '<select name="ltid'.$student12['st_id'].'" id="ltid"';
								 	if($ltid){
									echo 'disabled';}
                                      echo '><option value="">Select Leave Type</option>';
                                        ...
                                        echo '</select>';
										if($ltid){
									echo '<input type="hidden" id="ltid" name="ltid'.$student12['st_id'].'" class="text err" value="'.$ltid.'" />';}
									echo '<input type="hidden" id="apply" name="apply'.$student12['st_id'].'" class="text err" value="';
									if($staffleaveid){ echo $staffleaveid;}else { echo "0"; }
									echo '" />';
										echo '</td>
                        <td><center><input type="text" id="reason" name="reason'.$student12['st_id'].'" class="text err" value="'.$emp_display['l_des'].'" /></center></td>                        </tr>';
                     $att++; }
                   echo '</tbody>
                  </table></p><br><br>	                  	
        			</div>
