 ...
 if (isset($_POST['submit']))
{
	...
	$select_record=mysql_query("SELECT * FROM driver WHERE status='1'");
					...
					{ 
						...
						$select_record1=mysql_query("SELECT * FROM sattendance where (day='$date_day') AND m_id=$mid AND d_id=$d_id AND ay_id=$acyear");
						$queryfetch1=mysql_fetch_array($select_record1);
						
						if($queryfetch1){
							header("Location:datt_single.php?mid=$mid&msg=already");
						}else{
						   if(($postTest=='0' || $postTest=='off') && !$apply1){
							...
							$qry=mysql_query("INSERT INTO `sattendance` (`day`,`month`,`year`,`result`,`m_id`,`d_id`,`lt_id`,`reason`,`ay_id`,`result_half`,`l_apply`) VALUES ('$date_day', '$date_month','$date_year', '$postTest','$mid','$d_id','$ltid','$reason','$acyear','$resulthalf','$apply1')");
						}
					}					
		if(!$qry)
{
...
{
 header("Location:datt_single.php?mid=$mid&msg=succ");
}
}
 ?>
</head>
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
				...
				<li class="no-hover"><a href="datt_mng.php?mid=<?php echo $mid;?>" title="Result Management">Driver Attendance managemrnt</a></li>
                <li class="no-hover">Add Driver Attendance</li>
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1>Add New Drivers Attendance <b>( <?php echo $month['m_name'];?> )</b></h1>                
            ...
            <?php } ?>
				...
				<div class="block-border">
					<div class="block-header">
						<h1>Add New Driver Attendance <b>( <?php echo $month['m_name'];?> )</b></h1><span></span>
					</div>
...
<script defer src="js/zebra_datepicker.js"></script>
  <!-- end scripts-->
   ...
   $().ready(function() {
        ...
        $.get("dattenance.php",{value:value},function(data){
			$( "#test" ).html(data);
        });
