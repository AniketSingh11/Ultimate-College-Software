 ...
 <div id="wrapper">
     ...
     <div id="content">		
		 ...
		 <div id="content-header">
			 <h1> Other Workers Salary Detail <a href="ow_day_salary_detail_print.php?cid=<?php echo $_GET['cid'];?>" title="Print" target="_blank" ><button type="button" class="btn btn-success">Print</button></a> </h1><?php if($filter){?><span style="float:right; margin-right:30px;"><b>Filter by : </b> Employee Type = <?php echo $filter;?></span><?php } ?>
		 </div>  <!-- #content-header -->	
...
<?php } ?>		 <div class="row">
					 ...
					 <div class="portlet">
						 <div class="portlet-header">
                            ...
                            <div class="btn-group" style="float:right;">
                              ...
                              <li <?php if(empty($cid)){?> class="active"<?php }?>><a href="ow_day_salary_detail.php">All</a></li>
                              <?php 
							  		$classl = "SELECT * FROM others_category";
                                    $result1 = mysql_query($classl) or die(mysql_error());
									while ($row1 = mysql_fetch_assoc($result1)){
								?>
                                <li <?php if($cid==$row1['oc_id']){?> class="active"<?php }?>><a href="ow_day_salary_detail.php?cid=<?=$row1['oc_id']?>"><?=$row1['category_name']?></a></li><?php } ?>
							  </ul>
							</div>
						 </div>  <!-- /.portlet-header -->
						 <div class="portlet-content">
                            ...
                            {
							$emp_query="SELECT * FROM others where status='1' and relivestatus=0 and s_type='1' and category_id='$cid' order by fname asc";
                            }else{                                
                                $emp_query="SELECT * FROM others where status='1' and relivestatus=0 and s_type='1' order by fname asc";
                            }	
		...
		?>                             
										 <tr>
                                             ...
                                             <td><a href="daily_salary_detail1.php?id=<?php echo $emp_id; ?>&syear=<?php echo $ay['s_year'].'&eyear='.$ay['e_year'];?>" title="Salary Details"><button type="button" class="btn btn-sm btn-success">
        ...
        ?>						</tbody>
				...
				<?php
					if(!empty($cid) && $cid!="all")
                            {
							$emp_query="SELECT * FROM others where status='1' and relivestatus=0 and category_id='$cid' order by fname asc";
                            }else{                                
                                $emp_query="SELECT * FROM others where status='1' and relivestatus=0 and s_type='1' order by fname asc";
                            }							
		$emp_result=mysql_query($emp_query);
