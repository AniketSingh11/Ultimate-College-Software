...
$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setWidth(50);cellFont("D1","Cambria");

$objPHPExcel->getActiveSheet()->setCellValue("F1"," $name ");
$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setWidth(50);cellFont("F1","Cambria");
...
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Net Salary");$column++;
										...
										$emp_result1=mysql_query("select * from staff_allw_ded where type='Allowance' order by id asc");
										while($emp_display1=mysql_fetch_assoc($emp_result1))
										{
$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $emp_display1['name']);$column++;
										}
										$emp_result1=mysql_query("select * from staff_allw_ded where type='Deductions' order by id asc");
										while($emp_display1=mysql_fetch_assoc($emp_result1))
										{
$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $emp_display1['name']);$column++;
...
$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "LOB");$column++;

$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Loan Payment");$column++;

$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Advance Salary");$column++;

$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);cellFont("F2","Cambria");
$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, "Common Deduction");$column++;
    ...
    $column = 'A';
					$emp_id=$emp_display["st_ms_id"];	
								...
								$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, round($emp_display["n_salary"])); $column++;
										$emp_result1=mysql_query("select sum_id,amount from staff_month_salary_summary where st_ms_id=$emp_id and (st_id=$st_id AND o_id=$o_id AND d_id=$d_id) order by sum_id asc");
										while($emp_display1=mysql_fetch_assoc($emp_result1))
										{
											$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, $emp_display1['amount']); $column++;
										}
							    ...
							    $rowCount++;
										...
										while($emp_display1=mysql_fetch_assoc($emp_result1))
										{
											$content .= stripslashes($emp_display1['amount']). ',';
										}
										$content .= "\n";
							$count++;	
}
