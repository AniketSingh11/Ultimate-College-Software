...
$fixed=$salarylist['salary'];
							...
							?>                             
										 <tr>
                                             ...
                                             <td><a title="view" href="#styledModal<?php echo $emp_count.''.$emp_id;?>" data-toggle="modal"><img src="img/layout/view.png"/></a> <a title="Print" href="daily_salary_print.php?id=<?php echo $emp_id; ?>&stid=<?php if($st_id){ echo $st_id."&type=st";}else if($o_id){ echo $o_id."&type=ow";}else if($d_id){ echo $d_id."&type=dr";}?>&m=<?php echo $m_value;?>&syear=<?php echo $syear?>&eyear=<?php echo $eyear;?>" target="_blank"><img src="img/layout/print.png" /></a>
                                             <a title="Edit" href="daily_salary_edit.php?id=<?php echo $emp_id; ?>&stid=<?php if($st_id){ echo $st_id."&type=st";}else if($o_id){ echo $o_id."&type=ow";}else if($d_id){ echo $d_id."&type=dr";}?>&m=<?php echo $m_value;?>&syear=<?php echo $syear?>&eyear=<?php echo $eyear;?>" target="_blank"><img src="img/layout/edit.png"/></a>
											 <?php if($_SESSION['admin_type']=="0" || in_array("daily_salary.php", $permissions_record_delete) ){ ?> 
        ...
        <?php	
							$emp_query="select t1.others_id,t1.fname,t1.category_id,t1.email,t1.phone_no,t1.position from others t1 LEFT JOIN staff_daily_salary t2 ON t1.o_id = t2.o_id AND t2.month=$m_value AND t2.year=$y_value WHERE t2.o_id IS NULL AND t1.status='1' and t1.relivestatus='0' and t1.s_type='1' ";
        ...
        ?>		
        <?php	
							$emp_query="select t1.driver_id,t1.fname,t1.d_type,t1.email,t1.phone_no,t1.position from driver t1 LEFT JOIN staff_daily_salary t2 ON t1.d_id = t2.d_id AND t2.month=$m_value AND t2.year=$y_value WHERE t2.d_id IS NULL AND t1.status='1' and t1.relivestatus='0' and t1.s_type='1'";
							$emp_result=mysql_query($emp_query);
							while($emp_display=mysql_fetch_array($emp_result))
							{
