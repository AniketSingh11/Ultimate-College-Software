...
$d=date("Y-m-d");
//echo "SELECT * FROM staff WHERE STATUS =  '1' OR STR_TO_DATE( dor,  '%m/%d/%Y' ) BETWEEN STR_TO_DATE(  '$newDate',  '%Y-%m-%d' ) AND STR_TO_DATE(  '$d',  '%Y-%m-%d' ) order by fname asc";die;
include("header.php");
...
$type=$_GET["type"];
  ...
  if(isset($_POST["submit"]))
  {
	...
	$accno= mysql_real_escape_string($_POST["accno"]);	
	 ...
	 $extra_salary_type= mysql_real_escape_string($_POST["extra_salary_type"]);
...
$query="insert into staff_month_salary(st_id,o_id,d_id,staff_name,staff_id,date,day,month,year,g_salary,n_salary,d_total,loan_pay,total_leave,doj,position,role,accno,tleave,tlob,ay_id,a_id,date_day,date_month,date_year,extra_Salary,extra_salary_type,login_user_name)values('$stid','$oid','$did','$staff_name','$staff_id','$salary_date','$sdate_day','$sdate_month','$sdate_year','$gross_total','$net_salary','$deduction_total','$l_pay','$tleave','$doj','$position','$role','$accno','$tleave','$tlob','$acyear','$advace_id','$sdate_day1','$sdate_month1','$sdate_year1','$extra_salary','$extra_salary_type','$user')";
//die();
	...
	$emp_query1="select * from staff_allw_ded where type='Deductions' order by id asc";
										...
										while($emp_display1=mysql_fetch_array($emp_result1))
										{
											...
											if($pe_type=='1' || $pe_type=='2'){					
												...
												/* $lob_deduc=0;
													if($lob>0){
												$lob_deduc=round(($gross_total/$numdays)*$lob);
												//$deduc_total +=$lob_deduc;
												}
												echo $gross_total.''.$numdays.''$lob;die; */
												$lob_ded_salary1=$basicsalary-$lob;
												...
												if($mcpercent && $pe_type=='1'){
													...
													$pevalue=$lob_ded_salary*($mcpercent/100);
...
<div class="col-sm-12">
				            	<div class="form-group">
									...
									<select id="employee" name="employee" class="form-control" style="width:50%" onchange="select_employee()">
                                        ...
                                        <?php 
										//$emp_query="select * from staff WHERE status='1' or (dor BETWEEN $newDate AND $date) order by fname asc";
                                  ...
                                  <?php } 
								//  $emp_query="select * from others WHERE status='1' and s_type='0' or (STR_TO_DATE( dor,  '%m/%d/%Y' ) BETWEEN STR_TO_DATE(  '$newDate',  '%Y-%m-%d' ) AND STR_TO_DATE(  '$d',  '%Y-%m-%d' )) and relivestatus='0' order by fname asc";
								  $emp_query="select * from others WHERE status='1' and s_type='0' and relivestatus='0' order by fname asc";
										$emp_result=mysql_query($emp_query);
										while($emp_display=mysql_fetch_array($emp_result))
										{
                                  ...
                                  <?php } 
								//  $emp_query="select * from driver WHERE status='1' and s_type='0' or (STR_TO_DATE(dor,  '%m/%d/%Y') BETWEEN STR_TO_DATE('$newDate',  '%Y-%m-%d') AND STR_TO_DATE('$d',  '%Y-%m-%d')) and relivestatus='0' order by fname asc";
								  $emp_query="select * from driver WHERE status='1' and s_type='0' and relivestatus='0' order by fname asc";
										$emp_result=mysql_query($emp_query);
										while($emp_display=mysql_fetch_array($emp_result))
										{
