   ...
   $sdate_split1= explode('-', $value);
		 ...
		 $emp_query="select * from staff_month_salary where month=$sdate_month and year=$sdate_year  order by st_ms_id desc";	
							...
							while($emp_display=mysql_fetch_array($emp_result))
							{
								...
								if($st_id && mysql_num_rows(mysql_query("SELECT st_id FROM staff WHERE extra_salary_type=1 AND st_id=$st_id"))){
									$val=mysql_fetch_array(mysql_query("SELECT * FROM staff WHERE extra_salary_type=1 AND st_id=$st_id"));
								...
								if($o_id && mysql_num_rows(mysql_query("SELECT o_id FROM others WHERE extra_salary_type=1 AND o_id=$o_id"))){
									$val=mysql_fetch_array(mysql_query("SELECT * FROM others WHERE (extra_salary_type=1) AND o_id=$o_id"));
									$amt=$val["extra_salary"];
									$bank=2;
								...
								if($d_id && mysql_num_rows(mysql_query("SELECT d_id FROM driver WHERE extra_salary_type=1 AND d_id=$d_id"))){
									$val=mysql_fetch_array(mysql_query("SELECT * FROM driver WHERE extra_salary_type=1 AND d_id=$d_id"));
									$amt=$val["extra_salary"];
									$bank=2;
								}
...
//echo "select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_daily_salary where month=$sdate_month and year=$sdate_year order by st_ms_id desc";die;
