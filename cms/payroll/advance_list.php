		...
		$o_id=$_GET["o_id"];
		 ...
		 ?>
                    ...
                    while ($row_enq = mysql_fetch_array($emp_query)) {
					 ...
					 $emp_query1=mysql_query("select * from staff WHERE status='1' and relivestatus='0' AND st_id='$st_id'  order by fname asc");
							
								      $emp_display=mysql_fetch_array($emp_query1);
									
					 $emp_query2=mysql_query("select * from others WHERE status='1' and relivestatus='0' AND o_id='$o_id' order by fname asc");			 $emp_display1=mysql_fetch_array($emp_query2);
					
					$emp_query3=mysql_query("select * from driver WHERE status='1' and relivestatus='0' AND d_id='$d_id' order by fname asc");
		...
		?>  
			...
			?> 					   
										 <tr>
											 ...
											 <?php if($_SESSION['admin_type']=="0" || in_array("advance_list.php", $permissions_record_delete) ){ ?>
												<a title="delete" href="advance_delete.php?id=<?php echo $a_id;?>" onClick="return confirm('are you sure you wish to delete this record');"><img src="img/layout/delete.png"/></a> 
											  <?php } ?> 
