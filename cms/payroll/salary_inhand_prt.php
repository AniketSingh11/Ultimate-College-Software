...
$id=$_GET['id'];
$emp_rest=mysql_query("select * from salary_inhand where id=$id");
//echo "select * from staff_salary_report where id=$id"; die;
							$salary=mysql_fetch_array($emp_rest);
							
									$month=$salary["month"];
									$year=$salary["year"];	?>				
<div style="text-align:right; padding:10px 10px;">Date : <?php echo date("d/m/Y");?></div>
<h3>Salary In Hand Report ( <?php echo $month."-".$year?> )</h3>
...
//echo "select * from staff_salary_report where id=$id"; die;
							...
							$emp_query="select st_ms_id,st_id,o_id,d_id,n_salary,staff_name,position,accno,n_salary from staff_month_salary where month=$month and year=$year order by st_ms_id desc";	
