...
?>
		 <div id="content-header">
			 <h1> <?php echo $staff1['fname']." ".$staff1['lname']." - (".$syear."-".$eyear.")";?> ESI and PF Details <a href="emp_esi_pf_detail.php"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 </div>  <!-- #content-header -->	
				 ...
				 <div class="col-md-12">
					 <div class="portlet">
						 <div class="portlet-header">
                             ...
                             <a href="esi_pf_detail_down.php?id=<?php echo $st_id."&syear=".$syear."&eyear=".$eyear."&acid=".$acyear;?>" title="Download Report"><button type="button" class="btn btn-success">Download Report</button></a>                         
							  ...
							  while($row=mysql_fetch_array($qry))
								{
									$syear1=$row['s_year'];
									$eyaer1=$row['e_year'];?>
                                  <li <?php if($row['y_name']==$yearstring){ echo 'class="active"';}?>><a href="esi_pf_detail.php?id=<?php echo $st_id;?>&syear=<?php echo $syear1."&eyear=".$eyaer1;?>"><?php echo $row['y_name'];?></a></li>
                                  <?php } ?>                             		
							  </ul>
							</div>                            
                         </div>  <!-- /.portlet-header -->
						 <div class="portlet-content">
							...
							>
								  ...
								  $emp_query="SELECT DISTINCT a.* FROM staff_month_salary a, staff_month_salary_summary b WHERE a.st_id=$st_id AND ((a.year=$syear AND a.month>5) OR (a.year=$eyear AND a.month<=5)) AND a.st_ms_id = b.st_ms_id AND b.ad_id IN (".implode(',',$myarray).") AND b.pevalue AND b.amount"; 
