 ...
 <div id="wrapper">
     ...
     <div id="content">				
		 <div id="content-header">
			 <h1> Driver Salary Details <a href="driver_day_salary_detail_print.php?" title="Print" target="_blank" ><button type="button" class="btn btn-success">Print</button></a></h1><?php if($filter){?><span style="float:right; margin-right:30px;"><b>Filter by : </b> Driver Type = <?php echo $filter;?></span><?php } ?>
		 </div>  <!-- #content-header -->	
...
<?php } ?>		 <div class="row">
				 <div class="col-md-12">
					 <div class="portlet">
						 <div class="portlet-header">
							 <h3>
								Driver Salary Details List 
							 </h3>
                            <div class="btn-group" style="float:right;">
							  <button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown">
							    Filter by Driver Type <span class="caret"></span>
							  </button>
							  <ul class="dropdown-menu" role="menu">
                                <li <?php if(!$filter){ echo 'class="active"';}?>><a href="driver_day_salary_detail.php">Over All</a></li>
							    <li <?php if($filter=="Driver"){ echo 'class="active"';}?>><a href="driver_day_salary_detail.php?filt=Driver">Driver</a></li>
							    <li <?php if($filter=="Non-Driver"){ echo 'class="active"';}?>><a href="driver_day_salary_detail.php?filt=Non-Driver">Non-Driver</a></li>
							  </ul>
							</div>
						 </div>  <!-- /.portlet-header -->
									...
									<thead>
										<tr>
											<th data-sortable="true">S.No</th>
                                             ...
                                             <th data-filterable="false" data-sortable="true" >Action</th>
										</tr>
									</thead>
									<tbody>
										<?php	
										...
										if($filter=="Driver"){
											$emp_query="select * from driver where d_type='Driver' and s_type='1' and status='1' AND relivestatus=0 order by fname asc";
										}else if($filter=="Non-Driver"){
											$emp_query="select * from driver where d_type='Non-Driver' and s_type='1' and status='1' AND relivestatus=0 order by fname asc";
										}else{
											$emp_query="select * from driver where s_type='1' and status='1' AND relivestatus=0 order by fname asc";
										}
		...
		?>                             
										 <tr>
                                             ...
                                             <td><a href="day_salary_detail2.php?id=<?php echo $emp_id; ?>&syear=<?php echo $ay['s_year'].'&eyear='.$ay['e_year'];?>" title="leave Details"><button type="button" class="btn btn-sm btn-success">
        ...
        ?>							
				...
				<?php
					if($filter=="Driver"){
						$emp_query="select * from driver where  d_type='Driver' and status='1' AND relivestatus=0 order by fname asc";
					}else if($filter=="Non-Driver"){
						$emp_query="select * from driver where d_type='Non-Driver' and status='1' AND relivestatus=0 order by fname asc";
					}else{
