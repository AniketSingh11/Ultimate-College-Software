...
?>
 ...
 <div id="wrapper">
	   ...
	   include("sidebar.php");
	   $emp_query1="select * from driver where d_id='$sid'";
		$emp_result1=mysql_query($emp_query1);
		$staff1=mysql_fetch_array($emp_result1);
	   ?>
     <div id="content">	
		 <div id="content-header">
			 <h1><?php echo $staff1["fname"]." ".$staff1["lname"]; ?> - Leave Details  <a href="driver_leave_list.php"><button type="button" class="btn btn-success"> <i class="fa fa-angle-double-left"></i> Back</button></a></h1>
		 ...
		 <div id="content-container">
			 <div class="row">
				...
				<div class="portlet-content">	
                        ...
                        <a href="leave_detail_down.php?id=<?php echo $sid."&syear=".$syear."&eyear=".$eyear."&acid=".$acyear."&type=dr";?>" title="Download Report"><button type="button" class="btn btn-success">Download Report</button></a>
							  ...
							  while($row=mysql_fetch_array($qry))
								{
									$syear1=$row['s_year'];
									$eyaer1=$row['e_year'];?>
                                  <li <?php if($row['y_name']==$yearstring){ echo 'class="active"';}?>><a href="leave_detail2.php?syear=<?php echo $syear1."&eyear=".$eyaer1;?>&id=<?php echo $sid;?>"><?php echo $row['y_name'];?></a></li>
                                  <?php } ?>
							  </ul>
							</div>
                    </ul>
...
<div class="table-responsive"> <center><h4>Applied Leaves ( <?php echo $syear." - ".$eyear;?> )</h4></center>
							...
							>
									...
									<tbody>
										<?php
										$emp_query="select * from staff_leave where d_id=$sid AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5')) order by id desc";
		...
		while($emp_display=mysql_fetch_array($emp_result))
		{
			...
			$emp_id=$emp_display["d_id"];	
                                ...
                                <button class="btn btn-small btn-warning" >Pending</button><?php }else if($emp_display['status']=='1'){?><button class="btn btn-small btn-success" >Approved</button> <?php } else{?><button class="btn btn-small btn-primary">Rejected</button><?php } ?></td>
											 ...
											 <td><a title="view" href="#styledModal<?php echo $id.$emp_id;?>" data-toggle="modal"><img src="img/layout/view.png"/></a> <a title="edit" href="leave_detail_edit2.php?id=<?php echo $id; ?>"><img src="img/layout/edit.png"/></a> 
											<?php if($_SESSION['admin_type']=="0" || in_array("leave_detail2.php", $permissions_record_delete) ){ ?>
												<a title="delete" href="leave_delete3.php?id=<?php echo $id; ?>&stid=<?php echo $sid;?>" onClick="return confirm('are you sure you wish to delete this record');"><img src="img/layout/delete.png"/></a> 
											<?php } ?>
											 </td>
										 </tr>
		<?php 
		...
		while($emp_display=mysql_fetch_array($emp_result))
		{
			...
			$tleave=0;
			$emp_query1="select * from staff_leave where status='1' AND (d_id=$sid AND l_type=$lt_id) AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5'))";
			$emp_result1=mysql_query($emp_query1);
 ...
 <div id="staffModal" class="modal modal-styled fade">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <center><img class="thumbnail" src="../img/driver/<?php echo $staff1['photo']; ?>" alt="staff photo" width="200px" height="200px;"></center>
        <table class="table">
					        <tbody>
                            <tr>
                              ...
                              <tr>
					            <td>Employee ID</td>
					            <td>:</td>
					            <td><?php echo $staff1["driver_id"] ; ?></td>
					          </tr>
					          <tr>
					            <td>First Name</td>
					            <td>:</td>
...
<?php
										$emp_query1="select * from staff_leave where d_id=$sid AND ((year=$syear AND month>'5') OR (year=$eyear AND month<='5')) order by id desc";
		$emp_result1=mysql_query($emp_query1);
		$emp_count=1;
		while($emp_display1=mysql_fetch_array($emp_result1))
		{
			$id1=$emp_display1["id"];	
			$emp_id1=$emp_display1["d_id"];			
		?>  
