...
<img src="../img/printer.png"></a>Teaching Category : <span class="error">*</span></label>
                               <select id="d_type" name="d_type" onChange="stafftype()" class="required">
                                
                                <option value="">All</option>
                                <option value="Driver" <?php if($_GET['d_type']=="Driver"){ echo "selected"; }?>>Driver</option>
                                  <option value="Non-Driver" <?php if($_GET['d_type']=="Non-Driver"){ echo "selected"; }?>>Non_Driver</option>
...
<h3><?php
if($d_type=="Driver")
echo "DRIVER SALARY LIST";
else if($d_type=="Non-Driver")
echo "NON-DRIVER SALARY LIST";
else
echo "Driver Salary Details List"; 
                                  ...
                                  <th><center>Gender</center></th>
                                    ...
                                    <?php
									  ...
									  if($d_type)
									  {
										...
										$qry=mysql_query("SELECT * FROM driver where status='1' AND relivestatus=0 and d_type='$d_type' and s_type='1' ORDER BY fname asc");	
										
									  }
									  else
									  {
										  $qry=mysql_query("SELECT * FROM driver where status='1' AND relivestatus=0 and s_type='1' ORDER BY fname asc");	
									  }
							$count=1;
...
// Code that uses other library's $ can follow here.
  ...
  <script type="text/javascript">
	...
	function change_function() { 
     var cid =document.getElementById('d_type').value;
	 window.location.href = 'driver_salary_detail_print.php?d_type='+d_type;	  
	} 
  </script>
  <!-- end scripts-->
...
//alert(s_type);
window.location.href="driver_day_salary_detail_print.php?d_type="+document.getElementById('d_type').value;
