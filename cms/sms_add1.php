...
('$publish_day','$publish_month','$publish_year','$pdate','$title','$msg','$type','$bid','$acyear')";
				...
				if($result){
					if($type=='Staff'){
						...
						$select_record1=mysql_query("SELECT * FROM staff where s_type='Teaching' AND status='1'");
							...
							$url="http://api.unicel.in/SendSMS/sendmsg.php?";
								 $msg = "Dear Staffs, ".$msg.". GraceMHSS";
								 $ch = curl_init();
                                 curl_setopt($ch,CURLOPT_URL, $url);
                                 curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                 curl_setopt($ch, CURLOPT_POST, 1);
                                 curl_setopt($ch, CURLOPT_POSTFIELDS, "uname=chenna&pass=w$5Xs%286N&send=GRMHSS&dest=$phone_no&msg=$msg");
                                 $buffer = curl_exec($ch);
                                if(empty ($buffer))
                                {
					...
					{ 
					 ...
					 $url="http://api.unicel.in/SendSMS/sendmsg.php?";
								 $msg = "Dear Parents, ".$msg.". GraceMHSS";
								 $ch = curl_init();
                                 curl_setopt($ch,CURLOPT_URL, $url);
                                 curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                 curl_setopt($ch, CURLOPT_POST, 1);
                                 curl_setopt($ch, CURLOPT_POSTFIELDS, "uname=chenna&pass=w$5Xs%286N&send=GRMHSS&dest=$pphone_no&msg=$msg");
                                 $buffer = curl_exec($ch);
                                if(empty ($buffer))
                                {
									// echo " buffer is empty "; 
...
('$publish_day','$publish_month','$publish_year','$pdate','$title','$msg','Student','$bid','$cid','$sid','$acyear')";
				...
				if($result){
					$succ="";
									$error="";
								$qry2="SELECT * FROM parent where ay_id='$acyear' AND b_id='$bid' AND c_id='$cid'";
								if($sid && $sid>0){
									$qry2.=" AND s_id='$sid'";
								}						
								$select_record2=mysql_query($qry2);
