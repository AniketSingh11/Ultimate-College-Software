...
<style type="text/css">
 ...
 .table tr td.grade{background-color:#A2E17B; font-weight:bold;}
 .table tr td.mark{background-color:#FCC859;}
 .table tr td.first{background-color:#C5FC59;}
 </style>
</head>
			...
			<?php
				...
				if($cid && $sid && $eid){
							...
							$sectionlist=mysql_query("SELECT * FROM section WHERE s_id=$sid"); 
								  ...
								  if(!$subid){
								$qry7=mysql_query("SELECT a.* FROM subject a,subjectlist b WHERE (a.sl_id=b.sl_id) AND a.c_id=$cid AND a.s_id=$sid AND extra_sub=0 AND a.ay_id=$acyear ORDER BY b.sl_id ASC");
...
/*$scores = array(92, 84, 84, 83, 65, 41, 38, 38, 37 ,84);
$rank=getGrades($scores);
echo $rank['41'];*/


								  ?>
		<div class="grid_12">
				<h1><?php echo $class['c_name']."-".$section['s_name']." ".$exam['e_name'];?> Result </h1>
            ...
            <div class="grid_12">
				<div class="block-border">
					<div class="block-header">
                    	<h1><?php echo $class['c_name']."-".$section['s_name']." ".$exam['e_name'];?> Result </h1> 
                        <a href="result_mng_mark_excel.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&eid=<?php echo $eid;?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-success" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> Download Excel</button></a>                                              
                        <span></span>
					</div>
							  ...
							  while($row1=mysql_fetch_assoc($qry1))
								{
									$slid=$row1['sl_id'];
									   ...
									   $subjectype=$slist['s_type'];
									?>
									<th <?php //if($subjectype!='1'){ echo 'colspan="3"';}?> colspan="3"><?php echo $slist['s_name']; ?></th>
								<?php } ?>
                                    <th>Total</th>
                                    <th>result</th>
                                    <th>Rank</th>
						...
						?> 
							...
							$count=1;
							  while($row1=mysql_fetch_assoc($qry1))
								{	?>
									<td><b>Mark</b></td>
                                    <td><b>First<br>mark</b></td>
                                    <td><b>Place<br>ment</b></td>
							<?php  } ?>
                                    <td></td>
                                    <td></td>
                                    <td></td>
						...
						?> 
							...
							$gtotal=0;
							  while($row1=mysql_fetch_assoc($qry1))
								...
								if($subjectype=='1'){
								$studentlis2t=mysql_query("SELECT CONVERT(SUBSTRING_INDEX(mark,'-',-1),UNSIGNED INTEGER) AS mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear ORDER BY mark DESC LIMIT 0,1"); 
								 ...
								 if($paper=='1'){ ?>
                                 <td width="10%" <?php if($result) { if($result=="FAIL"){ echo 'style="background-color:#f32200; color:#FFF;border-top:dotted 1px #000;"'; }else{ echo 'class="mark"'; } }?>><?php if($total){ echo $mark."-".$mark1." = ".$total;}?> </td><?PHP $gtotal +=$total; } else {  ?>
								 <td width="10%" <?php if($result) { if($result=="FAIL"){ echo 'style="background-color:#f32200; color:#FFF;border-top:dotted 1px #000;"'; }else{ echo 'class="mark"'; } }?>><center><?php echo $row['mark'];?></center> </td>
								 <?php  $gtotal +=$row['mark']; }?>
                                 <td width="10%" class="first"><center><?php echo $topmark;?></center> </td>
                                 <td width="10%" class="grade"><center><?php echo $subjectrank[$row['mark']];?></center> </td>
								<?php }else{
									$studentlis2t=mysql_query("SELECT CONVERT(SUBSTRING_INDEX(fa_sa_mark,'-',-1),UNSIGNED INTEGER) AS fa_sa_mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear ORDER BY fa_sa_mark DESC LIMIT 0,1"); 
								   $topmarklist=mysql_fetch_assoc($studentlis2t);
                                ...
                                <td width="10%" class="first"><center><?php echo $topmark;?></center> </td>
                                <td width="10%" class="grade"><center><?php echo $subjectrank[$fa_sa_mark];?></center> </td>
                                <?php  $gtotal +=$total; } ?>
                <?php  } ?>
								...
								if($fail){ echo "FAIL"; $tfail++; }else{ echo "PASS"; $tpass++; } 
								if($toptotal && $toptotal<$gtotal){
									$toptotal=$gtotal;
								}else if(!$toptotal){
									$toptotal=$gtotal;
								}
								if($lowtotal && $gtotal<$lowtotal){
									$lowtotal=$gtotal;
								}else if(!$lowtotal){
									$lowtotal=$gtotal;
								}
							...
							$ttotal=0;
							  while($row1=mysql_fetch_assoc($qry1))
								{	
								   ...
								   $subjectype=$slist['s_type'];
							if($subjectype=='1'){
								$total=mysql_fetch_assoc(mysql_query("SELECT SUM(mark) as total_mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear"));
									$average=$total['total_mark']/($tpass+$tfail);
									$ttotal +=$total['total_mark'];
							}else{
									$total=mysql_fetch_assoc(mysql_query("SELECT SUM(fa_sa_mark) as total_mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear"));
									$average=$total['total_mark']/($tpass+$tfail);
									$ttotal +=$total['total_mark'];
							}
									?>
									<td colspan="3">
                                    	<table width="100%">
                                        	<tr>
                                            	<td><center><?=$total['total_mark']?></center></td>
                                                <td><center><?=round($average,2)?></center></td>
                                            </tr>
                                        </table>
                                    </td>
							<?php  }
								$taverage=$ttotal/($tpass+$tfail); 
							?>
                                    <td><table width="100%">
                                        	<tr>
                                            	<td><center><?=$ttotal?></center></td>
                                                <td><center><?=round($taverage,2)?></center></td>
                                            </tr>
                                        </table></td>
                                    <td></td>
                                    <td></td>
                                ...
                                <tr>
									<td></td>
                                    <td colspan="2">First Mark / Last Mark</td>
                                     <?php 
							$qry1=mysql_query("SELECT a.* FROM subject a,subjectlist b WHERE (a.sl_id=b.sl_id) AND a.c_id=$cid AND a.s_id=$sid AND a.ay_id=$acyear AND b.extra_sub=0 ORDER BY b.sl_id ASC");
							$count=1;
							  while($row1=mysql_fetch_assoc($qry1))
							...
							if($subjectype=='1'){
								$studentlis2t=mysql_query("SELECT CONVERT(SUBSTRING_INDEX(mark,'-',-1),UNSIGNED INTEGER) AS mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear ORDER BY mark DESC LIMIT 0,1"); 
								$studentlis3t=mysql_query("SELECT CONVERT(SUBSTRING_INDEX(mark,'-',-1),UNSIGNED INTEGER) AS mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear AND mark ORDER BY mark ASC LIMIT 0,1");
								$topmarklist=mysql_fetch_assoc($studentlis2t);
								   $topmark=$topmarklist['mark'];
								   $lastmarklist=mysql_fetch_assoc($studentlis3t);
								   $lastmark=$lastmarklist['mark'];
								$total=mysql_fetch_assoc(mysql_query("SELECT SUM(mark) as total_mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear ORDER BY mark DESC LIMIT 0,1"));   
							}else{
									...
									$studentlis3t=mysql_query("SELECT CONVERT(SUBSTRING_INDEX(fa_sa_mark,'-',-1),UNSIGNED INTEGER) AS fa_sa_mark FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid1 AND ay_id=$acyear AND fa_sa_mark ORDER BY fa_sa_mark ASC LIMIT 0,1"); 
									$topmarklist=mysql_fetch_assoc($studentlis2t);
								   $topmark=$topmarklist['fa_sa_mark'];
								   $lastmarklist=mysql_fetch_assoc($studentlis3t);
								   $lastmark=$lastmarklist['fa_sa_mark'];
								   }
								?>
									<td colspan="3">
                                    	<table width="100%">
                                        	<tr>
                                            	<td><center><?=$topmark?></center></td>
                                                <td><center><?=$lastmark?></center></td>
                                            </tr>
                                        </table>
                                    </td>
							<?php  } ?>
                                    <td><table width="100%">
                                        	<tr>
                                            	<td><center><?=$toptotal?></center></td>
                                                <td><center><?=$lowtotal?></center></td>
                                            </tr>
                                        </table></td>
                                    <td></td>
                                    <td></td>
                                ...
                                <tr>
									<td></td>
                                    <td colspan="2">No of Pass / No of Failure</td>
                                     <?php 
							$qry1=mysql_query("SELECT a.* FROM subject a,subjectlist b WHERE (a.sl_id=b.sl_id) AND a.c_id=$cid AND a.s_id=$sid AND a.ay_id=$acyear AND b.extra_sub=0 ORDER BY b.sl_id ASC");
							$count=1;
							  while($row1=mysql_fetch_assoc($qry1))
								...
								$subjectlist1=mysql_query("SELECT * FROM subjectlist WHERE sl_id='$slid' AND extra_sub=0"); 
								   ...
								   $paper=$slist['paper'];
								   $subjectype=$slist['s_type'];
								$lqry1="SELECT r_id FROM result WHERE c_id=$cid AND s_id=$sid AND e_id=$eid AND sub_id=$subid AND ay_id=$acyear";
				if($subjectype=='1'){
					$lqry1 .=" AND mark";
					$lqry2 = $lqry1." AND result='PASS'";
				}else{
					$lqry1 .=" AND fa_sa_mark";
					$lqry2 = $lqry1." AND fa_sa_grade!='E'";
				}
				$lqrys1=mysql_query($lqry1);
				$total_appeared=mysql_num_rows($lqrys1);
				$lqrys2=mysql_query($lqry2);
				$no_passed=mysql_num_rows($lqrys2);
				$no_failed=$total_appeared-$no_passed;?>
									<td colspan="3">
                                    	<table width="100%">
                                        	<tr>
                                            	<td><center><?=$no_passed?></center></td>
                                                <td><center><?=$no_failed?></center></td>
                                            </tr>
                                        </table>
                                    </td>
							<?php  } ?>
                            <td><table width="100%">
                                        	<tr>
                                            	<td><center><?=$tpass?></center></td>
                                                <td><center><?=$tfail?></center></td>
                                            </tr>
                                        </table></td>
                                    <td></td>
                                    <td></td>
				...
				$percent=($no_passed/$total_appeared)*100;
								?>
									<td colspan="3">
                                    	<table width="100%">
                                        	<tr>
                                            	<td colspan="2"><center><?=$percent." %"?></center></td>
                                            </tr>
                                        </table>
                                    </td>
							<?php  } 
							$tpercent=(($tpass/($tpass+$tfail))*100);?>
                                    <td><table width="100%">
                                        	<tr>
                                            <td><center><?php echo round($tpercent,2)." %"; ?><center>
                                            </td>
                                            </tr>
                                        </table></td>
                                    <td></td>
