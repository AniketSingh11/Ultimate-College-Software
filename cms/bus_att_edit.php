 ...
 if (isset($_POST['submit']))
{
	...
	if($eday!=$date_day){
		$select_record1=mysql_query("SELECT * FROM battendance where (day='$date_day') AND m_id=$mid AND r_id=$rid AND ay_id=$acyear");
						...
						if($queryfetch1){
							header("Location:bus_att_edit.php?rid=$rid&mid=$mid&bid=$bid&eday=$eday&msg=already");
							exit;
						}
	}
			$select_record=mysql_query("SELECT ss_id,c_id,s_id,b_id,sp_id,busfeestype,admission_number,firstname,lastname FROM student WHERE sp_id IN (".implode(',',$myarray).") AND ay_id=$acyear ORDER BY FIND_IN_SET(sp_id, '".implode(',',$myarray)."')");
					...
					{ 
						...
						if($check){
						$sql=mysql_query("UPDATE battendance SET day='$date_day',month='$date_month',year='$date_year',mresult='$postTest',r_id='$rid',b_id='$bid' WHERE ss_id='$stu_id' AND m_id='$mid' AND day=$eday");
						}else{
						...
						header("Location:bus_att_edit.php?rid=$rid&mid=$mid&bid=$bid&eday=$date_day&msg=succ");
						exit;
	}
 ?>
</head>
