    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover"><?php if($cid){ echo $class['c_name']." - "; }?>Admission Student List</li>                
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1><?php if($cid){ echo $class['c_name']." - "; }?>Admission Student List</h1>
                <a href="board_select_admin.php" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a>
                <a href="admission_single.php?bid=<?php echo $bid;?>" title="add" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">Main Admission</button></a>
                <a href="admission_new.php?bid=<?php echo $bid;?>" title="add"  style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">Impoprt student</button></a>
            ...
            <div class="grid_12">
				<div class="block-border" id="tab-panel-2">
					<div class="block-header">
						<h1><?php if($cid){ echo $class['c_name']." - "; }?>Admission Student List</h1>
                        <span></span>
                        <div class="_25" style="margin:5px 0 0 20px">
                            <a href="admission_down.php?<?php echo "bid=".$bid."&cid=".$cid."&acid=".$acyear."&gen=".$gen;?>" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-success"><img src="img/icons/packs/fugue/16x16/clipboard-invoice.png"> Download Report </button></a>
                        </div>
                        <div class="_25" style="margin:8px 0 0 20px">
                            <select name="gen" id="gen" onchange="download_function()">
                            	<option value='all'>All</option>
			  ...
			  while($row=mysql_fetch_array($qryselect))
        		{
					...
					$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid1"); 
                                ...
                                <td><center><?php echo $class['c_name']; if($sid){ echo "-".$section['s_name']; }?>
                                 ...
                                 <a href="admission_edit.php?ssid=<?php echo $row['ss_id'];?>&bid=<?php echo $bid;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="studentmng_delete1.php?ssid=<?php echo $row['ss_id']; ?>&cid=<?php echo $cid; ?>&sid=<?php echo $sid;?>&bid=<?php echo $bid;?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
...
function change_function() { 
	 ...
	 if(cid != 'all'){
	  window.location.href = 'admission.php?cid='+cid+'&bid=<?php echo $bid;?>';	  
	 } else {
	 window.location.href = 'admission.php?bid=<?php echo $bid;?>';	  
	 }
	}
	function change_function1() { 
     var bid =document.getElementById('bid').value;
	 window.location.href = 'admission.php?bid='+bid;	  
	}  
	function download_function() { 			
     var down =document.getElementById('gen').value;
	 if(down){
	window.location.href ='admission.php?gen='+down+'<?php echo "&bid=".$bid."&cid=".$cid;?>';	  
	 }
	}
	</script>
