    ...
    <?php 
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover"><a href="board_select_homework.php" title="<?php echo $board['b_name'];?>"><?php echo $board['b_name'];?></a></li>
				<li class="no-hover">Homework Management</li>                
			</ul>
		...
		<div id="main-content">
            ...
            <a href="board_select_homework.php" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a>
							...
							<p>
								<label for="select">Month : <span class="error">*</span></label>
									...
									$mcount=1;
                                            ...
                                            echo '<select name="mid" id="mid" class="required"> <option value="">Select Month</option>';
			...
			<?php
				...
				if($cid && $sid && $mid){
							...
							$subjectlist=mysql_query("SELECT * FROM subject WHERE sub_id=$subid"); 
								  ...
								  ?>
		<div class="grid_12">
				<h1><?php echo $class['c_name']."-".$section['s_name']." (".$month['m_name']." ) ";?> Homework <b><?php if($subid){?>(<?php echo $slist1['s_name'];?>)<?php } ?></b></h1>
                 <?php 
			  ...
			  while($row1=mysql_fetch_array($qry1))
        		{
					if(!$subid){
						$subid1=$row1['sub_id'];
						header("Location:homework_mng.php?cid=$cid&sid=$sid&mid=$mid&subid=$subid1&bid=$bid");
						exit;
					}
					$slid=$row1['sl_id'];
					...
					?>
                 <a href="homework_mng.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $row1['sub_id']; ?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-error" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> <?php echo $slist['s_name']; ?></button></a>
                <?php }  } ?>
                <br>
                <br>
                <?php if($cid && $subid && $sid){?>
                <a href="homework_single.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" title="add" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">  Add Homework Detail</button></a>
                <?php $msg=$_GET['msg'];
				if($msg=="dsucc"){?>
			  ...
			  while($row=mysql_fetch_array($qry))
        		{
						?>
                                ...
                                <td width="80px;">
                                 ...
                                 <a href="homework_edit.php?hid=<?php echo $row['h_id'];?>&cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="homework_delete.php?hid=<?php echo $row['h_id']; ?>&cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
                                 </td>
								</tr> 
                                <div id="info-dialog<?php echo $count;?>" title="<?php echo $class['c_name']."-".$section['s_name']." (".$month['m_name']." ) ";?>HomeWork <b><?php if($subid){?>(<?php echo $slist1['s_name'];?>)<?php } ?></b>" style="display: none;">
				<p>Subject : <strong><?php echo $slist1['s_name'];?></strong></p>
...
// Code that uses other library's $ can follow here.
  ...
  <script type="text/javascript">
	...
	function change_function() { 
	 ...
	 window.location.href = 'homework_mng.php?bid='+cid;	  
	} 
  </script>
  <!-- end scripts-->
