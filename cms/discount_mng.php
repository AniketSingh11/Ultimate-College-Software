    ...
    <div id="main" role="main">
		...
		<div id="main-content">
			...
			<div class="grid_12">
                ...
                <a href="discount_add.php?cid=<?php echo $_GET['cid'];?>&bid=<?php echo $bid;?>"  title="add" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">  Add Discount</button></a>
				<span style="margin-left:20px;"><a href="std_discount.php?cid=<?php echo $_GET['cid'];?>&bid=<?php echo $bid;?>" style="width:100px"title="Print" ><button class="btn btn-small btn-green"><img src="img/icons/packs/fugue/16x16/inbox-download.png">Download Report</button></a></span>
                ...
                <div class="alert success"><span class="hide">x</span>Your Record Successfully Deleted !!!</div>
                 <?php } 
				 $qry="SELECT * FROM discount WHERE bid=$bid AND ay_id=$acyear";
				 if($cids){
					 $qry .=" AND c_id=$cids";
				 }
			  ...
			  while($row=mysql_fetch_array($qry))
				...
				//echo "SELECT c_name FROM class WHERE c_id=$cid";die;
							...
							$sectionlist=mysql_query("SELECT s_name FROM section WHERE s_id=$sid"); 
								  ...
								  $dislist=mysql_num_rows(mysql_query("SELECT dv_id FROM discount_value WHERE d_id=$did AND payment=0")); 
								  if($dislist>0){
									  $status=0;
								  }else{
									  $status=1;
				...
				?>
                                ...
                                <td class="view"><center><a href="discount_detail.php?did=<?=$did?>&cid=<?=$cids?>&bid=<?=$bid?>"><button class="btn btn-primary btn-small"><img src="img/icons/packs/fugue/16x16/pictures-stack.png"> View</button></a></center></td>
								<td width="10%">
                                <?php if($status=='0'){ 
                                	if($row['status']!='1') {?>
                                 <a href="discount_edit.php?did=<?=$did?>&cid=<?=$cids?>&bid=<?=$bid?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="discount_delete.php?did=<?=$did?>&cid=<?=$cids?>&bid=<?=$bid?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
...
// Code that uses other library's $ can follow here.
  ...
  <script type="text/javascript">
	...
	function change_function() { 
	 ...
	 window.location.href = 'discount_mng.php?bid='+cid;	  
	} 
	function change_function1() { 
     var cid =document.getElementById('cid').value;
	  window.location.href = 'discount_mng.php?cid='+cid+'&bid=<?php echo $bid;?>';	  
}
  </script>
  <!-- end scripts-->
