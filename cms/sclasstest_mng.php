...
<body id="top">
 ...
 <!--! end of #search-bar -->
    ...
    <?php 
				...
				if($cid && $sid && $subid){
							...
							if($mid){
								$examlist=mysql_query("SELECT * FROM month WHERE m_id=$mid"); 
								  ...
								  ?>
            <a href="sboard_select_classtest.php?bid=<?php echo $bid;?>" style="margin:3px 0 0 20px;"><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a>
            <?php 
			$monthno=date("m");
			  ...
			  while($row1=mysql_fetch_array($qry1))
        		{
					$mno=$row1['m_no'];
					if($mcount==1){?>
                 <a href="sclasstest_mng.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $row1['m_id'];?>&subid=<?php echo $subid; ?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-small btn-error" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> <?php echo $row1['m_name']; ?></button></a>
                <?php } if($mno==$monthno){
						$mcount=0;
					} }
					?>
		...
		<div class="grid_12">
				<h1><?php echo $class['c_name']."-".$section['s_name']." (".$month['m_name']." ) ";?> Class Test <b><?php if($subid){?>(<?php echo $slist['s_name'];?>)<?php } ?></b></h1>
                <?php if($cid && $mid && $sid){?>
                <a href="sclasstest_single.php?cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" style="margin:0px 0 0 10px;"><button class="btn btn-orange btn-small "><img src="img/icons/packs/fugue/16x16/user--plus.png">  Add Class Test</button></a>
                <?php $msg=$_GET['msg'];
				if($msg=="dsucc"){?>
				...
				<div class="block-border">
					...
					<div class="block-content">
						<table id="table-example" class="table">
							...
							<tbody>
                            <?php 
							$qry=mysql_query("SELECT * FROM classtest WHERE c_id=$cid AND s_id=$sid AND m_id=$mid AND sub_id=$subid AND ay_id=$acyear");
							$count=1;
			  while($row=mysql_fetch_array($qry))
        		{
						?>
                                ...
                                <td width="80px;">
                                 ...
                                 <a href="sclasstest_edit.php?ctid=<?php echo $row['ct_id'];?>&cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" title="Edit"><img src="./img/edit.png" alt="edit"></a>
                                 <a href="sclasstest_delete.php?ctid=<?php echo $row['ct_id']; ?>&cid=<?php echo $cid;?>&sid=<?php echo $sid;?>&mid=<?php echo $mid;?>&subid=<?php echo $subid;?>&bid=<?php echo $bid;?>" class="delete" title="delete" onClick="return confirm('are you sure you wish to delete this record');"><img src="./img/del.png" alt="delete"></a>
                                 </td>
								</tr> 
