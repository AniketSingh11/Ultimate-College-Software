...
<center>
<img src="img/logo_sms.png" title="latterpad" width="30%" />
</center>
...
top:120px; border:1px solid #000000;"><img src="./img/student/<?php echo strtoupper($row['photo']);?>" alt="student photo" width="100" height="100"></div>
...
<td width="30%"><b><?php echo $class['c_name']; if($sid){ echo "-".$section['s_name'];}?></b></td>
<td style="border:none;"></td>
</tr>
<tr>
<td width="5%">6</td>
...
<?php 
$qry=mysql_query("select * from sibling where p_id='$row[p_id]' and ay_id='$row[ay_id]'");
$cc=1;
while($row1=mysql_fetch_array($qry))
{
    if($row1[ss_id]==$ssid){}else{
    $studentlist=mysql_query("SELECT * FROM student WHERE ss_id=$row1[ss_id]");
    $row2=mysql_fetch_array($studentlist);
    
    $classlist=mysql_query("SELECT * FROM class WHERE c_id=$row2[c_id]");
    $class1=mysql_fetch_array($classlist);
    
        $sectionlist=mysql_query("SELECT * FROM section WHERE s_id=$row2[s_id]");
        $section1=mysql_fetch_array($sectionlist);
    
?>
<tr>
<td height="20px"><?=$cc?></td>
<td width="40%"><?=$row2['firstname']." ".$row2['middlename']." ".$row2['lastname']?></td>
<td><?=$row2['dob']?></td>
<td><?=$class1['c_name']."-".$section1['s_name'];?></td>
<td></td>
</tr>
<?php $cc++; }} ?>
<tr>
