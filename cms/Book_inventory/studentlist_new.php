			...
			$count=0;
	         while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE)
	         {
	          ...
	          //It wiil insert a row to our subject table from our csv file`
	           $sql = "INSERT into student (`ss_roll`, `ss_name`,s_id, `c_id`) 
	            	values('$emapData[1]','$emapData[2]','$sid','$cid')";
	         //we are using mysql_query function. it returns a resource on true else False on error
	         $result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
				if(! $result )
				{
					
						header("Location:studentlist_new.php?sid=$sid&cid=$cid&msg=eonfile");
						exit;
				}
				header("Location:studentlist_new.php?sid=$sid&cid=$cid&msg=succ");
				}
				$count++;
	         }
	         fclose($file);
