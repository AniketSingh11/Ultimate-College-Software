...
if (isset($_POST['submit']))
{
	...
	$ssid=$_POST['ssid'];
		...
		$qry=mysql_query("UPDATE student SET ss_roll='$ss_roll',ss_name='$ss_name',ss_gender='$ss_gender' WHERE ss_id='$ssid'");
    if($qry){
        header("Location:studentlist_edit.php?sid=$sid&cid=$cid&ssid=$ssid&msg=succ");
    }
    exit;
}
			...
			<div class="grid-24">
				...
				<div class="widget">
						...
						<div class="widget-header">
                         <a href="studentlist.php?sid=<?php echo $_GET['sid'];?>&cid=<?php echo $_GET['cid'];?>" style="margin:3px 0 0 20px;"><button class="btn btn-primary ">Back</button></a>
							<span class="icon-article"></span>
						...
						<div class="widget-content">
							...
							<form class="form uniformForm validateForm" method="post" action="" >
								...
								<div class="field-group">
									...
									<div class="field">
										<input type="text" name="roll" id="roll" size="32" value="<?php echo $student['ss_roll'];?>" class="validate[required]" />	
									</div>
								</div> <!-- .field-group -->
                                 <div class="field-group">
									<label for="required">Student Name:</label>
									<div class="field">
										<input type="text" name="name" id="name" size="32" value="<?php echo $student['ss_name'];?>" class="validate[required]" />	
									</div>
								</div> <!-- .field-group -->
                                <div class="field-group">		
									...
									<div class="field">
										<select name="gender" id="gender" class="validate[required]">
											<option value="M" <?php if($student['ss_gender']=='M'){ echo 'selected'; }?>>Male</option>
								<option value="F" <?php if($student['ss_gender']=='F'){ echo 'selected'; }?>>Female</option>										
										</select>										
									</div>		
								</div> <!-- .field-group -->
