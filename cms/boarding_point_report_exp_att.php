...
while($sprow = mysql_fetch_array($spresult, MYSQL_ASSOC))
{
$qry=mysql_query("SELECT * FROM staff WHERE sp_id='$sprow[stop_id]' ORDER BY FIND_IN_SET(sp_id, '".implode(',',$myarray)."')");
			  while($row=mysql_fetch_array($qry))
        		{
...
header("Content-Disposition: attachment;filename=Bus_BoardingPointDetails-list($sdate_day-$sdate_month-$sdate_year).xls");
