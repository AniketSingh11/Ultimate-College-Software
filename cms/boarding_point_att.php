    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Today Boarding Point List</li>
			</ul>
		...
		<div id="main-content">        		
		  ...
		  $startdate= $sdate_year.$sdate_month.$sdate_day;
							...
							$sdid=$class['sd_id'];
								...
								$driver1=mysql_fetch_array($driverlist1);
								 ...
								 $myarray = array();
								  ...
								  ?>
			<div class="grid_12">
				<h1><?php  echo $class['r_name'];?> - Route Student/Staff Boarding Point (<?php echo $sdate_day."/".$sdate_month."/".$sdate_year; ?>)</h1>
                <a href="boarding_point_report_exp_att.php?rid=<?php echo $rid;?>&ayid=<?php echo $acyear;?>&date=<?php echo $date;?>" style="margin-left:10px;"><button class="btn btn-small btn-success" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> Download Report</button></a>
                <a href="boarding_point_pdf_att.php?rid=<?php echo $rid;?>&ayid=<?php echo $acyear;?>&date=<?php echo $date;?>" style="margin-left:10px;"><button class="btn btn-small btn-success" ><img src="img/icons/packs/fugue/16x16/table--plus.png"> Pdf Report</button></a>
            ...
            <div class="grid_12">
            	<div class="block-border">
					<div class="block-header">
                    	<h1><?php  echo $class['r_name'];?> - Route Stop Details (<?php echo $sdate_day."/".$sdate_month."/".$sdate_year; ?>)</h1>
                        <span></span>
					</div>
			  ...
			  while($row=mysql_fetch_array($qry))
				...
				$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid"); 
							...
							$select_record2=mysql_query("SELECT * FROM attendance WHERE c_id=$cid AND s_id=$sid AND day=$sdate_day AND month=$sdate_month AND year=$sdate_year AND ay_id=$acyear AND ss_id=$ssid AND (result=1 OR (result='off' AND result_half='M'))");
					$monthday=mysql_fetch_array($select_record2);	
					$attid=$monthday['att_id'];
						if($attid){  
								  ?>
								<tr class="gradeX">
							...
							$qry=mysql_query("SELECT * FROM staff WHERE sp_id IN (".implode(',',$myarray).")");
			  while($row=mysql_fetch_array($qry))
        		{
								  $spid1=$row['sp_id'];
								  $stid=$row['st_id'];
								  ...
								  $fesstypearray1=array("Regural Bus","Sp.Bus","Onetime Sp.Bus","Onetime Bus"); 
								  $busfeestype1=$row['busfeestype'];
								  $select_record2=mysql_query("SELECT * FROM sattendance WHERE day=$sdate_day AND month=$sdate_month AND year=$sdate_year AND st_id=$stid AND (result=1 OR (result='off' AND result_half='M'))");
					$monthday=mysql_fetch_array($select_record2);	
					$attid=$monthday['satt_id'];
						if($attid){ 
								  ?>
								<tr class="gradeX">
									<td class="sno center"><center><?php echo $count; ?></center></td>
