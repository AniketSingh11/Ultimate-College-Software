...
{	
	  ...
	  $remark=$_POST['remark'];
	 	if($total){		  
	 $sql="INSERT INTO discount_others (day,month,year,cdate,total,ss_id,c_id,s_id,stype,cby,bid,ay_id,status,s_name,admin_no,remark) VALUES
('$day','$month','$year','$idate','$total','$ssid1','$cid1','$sid1','$stype','$user','$bid1','$acyear','0','$ss_name','$admin_no','$remark')";
...
('$lastid','$ssid1','4','$fgdid','$name','$amount','$type','0','$bid1','$acyear')";
	...
	header("location:discount_add_others.php?cid=$cids&bid=$bid&msg=succ");			 
	}
		}else{
			header("location:discount_add_others.php?cid=$cids&bid=$bid&id=$ssid1&msg=ferr");
		}
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Add Discount <?php if($cids){ echo "(".$class['c_name'].")";} ?></li>
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1>Add Discount <?php if($cids){ echo "(".$class['c_name'].")";} ?></h1>                
							...
							<p>
                               ...
                               <select name="n_sid" id="n_sid"  data-required="true"  class='required'  onchange="change_function2(this.value)"  style="width:90%">
									<option value="">Plese select Student </option>
										...
										$dislist=mysql_num_rows(mysql_query("SELECT d_id FROM discount_others WHERE ss_id=$ss_id AND status=0")); 
								  if($dislist>0){
									  $status=0;
								  }else{
									  $status=1;
								  ...
								  if($status==1){
										 		?>
                                        <option value="<?php echo $ss_id;?>" <?php if($ss_id==$id){ echo "selected"; }?>><?php echo  $emp_display["admission_number"]."-".$emp_display["firstname"]." ".$emp_display["lastname"]."(".$class.")"; ?></option>
                                  <?php }  }?>								
          ...
          <form id="validate-form" class="block-content-invoice form" action="" method="post">
                    ...
                    <?php 
					   ...
					   <table id="table-example" class="table">
							...
							<tr style="border:1px #B7B7B7 dotted;">
                                    ...
                                    <td><?php
                                    	$ss_id=$student['ss_id'];
                                    	$invoice=mysql_query("SELECT * FROM finvoice_others WHERE ss_id=$ss_id and c_status!=1 and i_status!=1");
                                    	$amt=0;
                                    	$distot=0;
                                    ...
                                    <?php 
                                    	if($pending==0){
                                    		...
                                    		echo '<input id="fees31" class="biginput txt text" type="text" max='.$pending.' autocomplete="off" value="" name="fees31">';
                                    	}
                                    ?>
                                    </td>
                                    </tr>
                ...
                <a href="discount_add_others.php?bid=<?=$bid?>&cid=<?=$cids?>" ><input type="button" value="cancel/Close" class="btn btn-red" style="width:120px"></a>&nbsp;&nbsp;
...
<script type="text/javascript">
	...
	function change_function1() { 
     var cid =document.getElementById('cid').value;
	  window.location.href = 'discount_add_others.php?cid='+cid+'&bid=<?php echo $bid;?>';	  
}
function change_function2(n){ 
		 window.location.href = 'discount_add_others.php?bid=<?php echo $bid;?>&cid=<?php echo $cids;?>&id='+n;
