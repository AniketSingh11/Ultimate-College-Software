 ...
 if (isset($_POST['submit']))
{
	...
	$select_record1=mysql_query("SELECT * FROM battendance where (day='$date_day') AND m_id=$mid AND r_id=$rid AND ay_id=$acyear");
						...
						if($queryfetch1){
							header("Location:bus_att_single.php?rid=$rid&mid=$mid&bid=$bid&msg=already");
						}else{
	...
	$select_record=mysql_query("SELECT ss_id,c_id,s_id,b_id,sp_id,busfeestype,admission_number,firstname,lastname FROM student WHERE sp_id IN (".implode(',',$myarray).") AND ay_id=$acyear ORDER BY FIND_IN_SET(sp_id, '".implode(',',$myarray)."')");
					...
					{ 
					    ...
					    $qry=mysql_query("INSERT INTO `battendance` (`day`,`month`,`year`,`mresult`,`r_id`,`sp_id`,`m_id`,`ss_id`,`b_id`,`ay_id`) VALUES ('$date_day', '$date_month','$date_year','$postTest','$rid','$spid1','$mid','$stu_id','$bid','$acyear')");
...
{
 header("Location:bus_att_single.php?rid=$rid&mid=$mid&bid=$bid&msg=succ");
}
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
                ...
                <li class="no-hover">Add Attendance - <?php echo $class['r_name'];?></li>
			</ul>
		...
		<div id="main-content">
			...
			<div class="grid_12">
				<h1>Add <?php echo $class['r_name'];?> - Student Attendance <b>( <?php echo $month['m_name'];?> )</b></h1>                
            ...
            <?php } ?>
				...
				<div class="block-border">
					<div class="block-header">
						<h1>Add <?php echo $class['r_name'];?> - Student Attendance <b>( <?php echo $month['m_name'];?> )</b></h1><span></span>
					</div>
			  ...
			  while($queryfetch=mysql_fetch_array($qry))
        		{
					...
					$classlist=mysql_query("SELECT * FROM class WHERE c_id=$cid"); 
                        ...
                        <td><center><input type="radio" id="name"  name="<?php echo $queryfetch['ss_id']?>" class="text err" value="1" checked="checked"  /></center></td>
                        <td><center><input type="radio" id="name" name="<?php echo $queryfetch['ss_id']?>" class="text err" value="0" /></center></td>
                        </tr>
                     <?php $att++; } ?>
