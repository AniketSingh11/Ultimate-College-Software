...
$others_amt=mysql_query("SELECT * FROM others_bill_all where ay_id=$acyear and board=$bid"); 
$tot_others=array();
while ($row = mysql_fetch_assoc($others_amt)) {
    $tot_others[]=$row;
}
if($_POST){
...
//echo "</pre>";
//die;
$del="delete from others_bill_all where board=$board";
$result1 = mysql_query($del) or die("Could not insert data into DB: " . mysql_error());
$result='';
$i=0;

foreach ($_POST['standanrd'] as $tmp) {	
	$amt=$_POST['amount'][$i];
	$id=$_POST['others_id'][$i];
	$gname=$_POST['g'][$i];

if($id=='')
	$sql="INSERT INTO others_bill_all (board,std,amount,gname,ay_id) VALUES ($board,$tmp,'$amt','$gname',$acyear)";
else
	$sql="INSERT INTO others_bill_all (others_id,board,std,amount,gname,ay_id) VALUES ($id,$board,$tmp,'$amt','$gname',$acyear)";
	$result = mysql_query($sql) or die("Could not insert data into DB: " . mysql_error());
//$lastid=$id = mysql_insert_id();
	$i++;

}
header("Location:others.php?status=ok");
}
?>
<style type="text/css">
.block-content {
    ...
    <div id="main" role="main">
		...
		<div id="title-bar">
			<ul id="breadcrumbs">
				...
				<li class="no-hover">Othes Fees Paymant</li> 
			</ul>
		...
		<div id="main-content">
            ...
            <div class="_25" style="float:right">
				...
				<h1><a href="<?php if($_GET['roll']){ echo "billing.php?bid=".$bid;}else{ echo 'board_select_fees.php';}?>" ><button class="btn btn-small btn-primary"><img src="img/icons/packs/fugue/16x16/arrow-curve-180-double.png"> Back </button></a> Books, Notes & Other Items</h1>
				<?php 
if($_GET['status']=="ok"){
echo '<div class="alert success"><span class="hide">x</span>Your Record Successfully Added! </div>';
}
?>
...
<!-- *************************** Old Student Total End ************************** -->
        ...
        <div class="field-group">
                            ...
                            <?php
                             $classl = "SELECT c_id,c_name FROM class WHERE b_id=$bid AND ay_id=$acyear";
											...
											if($row1['c_name']!="XI" && $row1['c_name']!="XII") {
												echo '<td><center>'; 
											if($cids==$row1['c_id']){
                                                echo "<div class='_75'><p><label>".$row1['c_name']."</label></p></div>";
												} else {
												echo "<div class='_75'><p><label>".$row1['c_name']."</label></p></div>";
												}
												echo '</center></td>';
                                        	...
                                        	echo '<tr>';
                                        	$class2 = "SELECT c_id,c_name FROM class WHERE b_id=$bid AND ay_id=$acyear";
                                            $result2 = mysql_query($class2) or die(mysql_error());
                                            while ($row2 = mysql_fetch_assoc($result2)){
                                           	if($row2['c_name']!="XI" && $row2['c_name']!="XII") {
                                            if(!empty($tot_others[$i]['others_id'])){
                                            echo '<td><input type="hidden"  name="others_id[]" value='.$tot_others[$i]['others_id'].'>';
                                        	} else {
                                        	echo '<td><input type="hidden"  name="others_id[]" value="">';	
                                        	}
                                        	echo '<input type="hidden" name="g[]" value="">';
                                            if($tot_others[$i]['amount']!= 0){
                                            	echo '<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value='.$row2['c_id'].' type="hidden" /><input name="amount[]" value='.$tot_others[$i]['amount'].' type="text" /></p></div></td>';
                                            } else {
                                            echo '<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value='.$row2['c_id'].' type="hidden" /><input name="amount[]" value="" type="text" /></p></div></td>';
                                        }
                                    } 
                            ...
                            <tr>
                            <?php 
                             $class3 = "SELECT c_id,c_name FROM class WHERE b_id=$bid AND ay_id=$acyear";
                                            $result3 = mysql_query($class3) or die(mysql_error());
                                           
											while ($row3 = mysql_fetch_assoc($result3)){ ?>
											<?php if($row3['c_name']!="XI" && $row3['c_name']!="XII") { ?>
												
											<?php } else { ?>
											<td colspan="5"> <?php echo $row3['c_name'];?><br>
											</td>
											<?php } 
							...
							<?php
							$i=$i-2;
							$class4 = "SELECT c_id,c_name FROM class WHERE b_id=$bid AND ay_id=$acyear";
							$result4 = mysql_query($class4) or die(mysql_error());
                                           	$j=1;
											while ($row4 = mysql_fetch_assoc($result4)){ ?>
											<?php if($row4['c_name']!="XI" && $row4['c_name']!="XII") { ?>
												
											<?php } else { ?>
											<td>
											<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value="<?= $row4['c_id']?>" type="hidden" /><input name="amount[]" value="<?= $tot_others[$i]['amount']?>" type="text" /><input name="g[]" value="I Group" type="hidden" /></p></div>
											<?php $i++; ?>
											</td>
											<td>
											<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value="<?= $row4['c_id']?>" type="hidden" /><input name="amount[]" value="<?= $tot_others[$i]['amount']?>" type="text" /><input name="g[]" value="II Group" type="hidden" /></p></div>
											<?php $i++; ?>
											</td>
											<td>
											<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value="<?= $row4['c_id']?>" type="hidden" /><input name="amount[]" value="<?= $tot_others[$i]['amount']?>" type="text" /><input name="g[]" value="III Group" type="hidden" /></p></div>
											<?php $i++; ?>
											</td>
											<td>
											<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value="<?= $row4['c_id']?>" type="hidden" /><input name="amount[]" value="<?= $tot_others[$i]['amount']?>" type="text" /><input name="g[]" value="IV Group" type="hidden" /></p></div>
											<?php $i++; ?>
											</td>
											<td>
											<div class="_70"><p style="margin-top:5px;padding-right: 10px;"><input name="standanrd[]" value="<?= $row4['c_id']?>" type="hidden" /><input name="amount[]" value="<?= $tot_others[$i]['amount']?>" type="text" /><input name="g[]" value="V Group" type="hidden" /></p></div>
											<?php $i++; ?>
											</td>
											<?php $j++; ?>
                            ...
                            <input type="hidden" id="boardHidden" name="board" value="<?= $board['b_id']?>">
