...
{	
    ...
    if($total==$total_pending)
	 ...
	 $sql="UPDATE discount SET day='$day',month='$month',year='$year',cdate='$idate',total='$total',remark='$remark',status='$status' WHERE d_id='$did'";
	...
	if($result){
				if($sfood){
						...
						$discountlist1=mysql_query("SELECT dv_id FROM discount_value WHERE d_id=$did AND ss_id=$ssid1 AND fg_id='1' AND fgd_id=$fgdid AND type='terms'");
								  $discount1=mysql_fetch_array($discountlist1);	
								  $dv_id=$discount1['dv_id'];
		if($amount && $discount1){
						$sql1=mysql_query("UPDATE discount_value SET amount='$amount' WHERE dv_id='$dv_id'") or die("Could not update data into DB: " . mysql_error());
		}else if(!$amount && $discount1){
			$delete=mysql_query("Delete from discount_value where dv_id='$dv_id'") or die("Could not delete data into DB: " . mysql_error());
		}else if($amount && !$discount1){
				 ...
				 while($row2=mysql_fetch_array($qry2))
					{
						...
						$discountlist1=mysql_query("SELECT dv_id FROM discount_value WHERE d_id=$did AND ss_id=$ssid1 AND fg_id=4 AND fgd_id=$fgdid  AND type='other'");
							$discount1=mysql_fetch_array($discountlist1);
								  $dv_id=$discount1['dv_id'];
...
('$did','$ssid1','4','$fgdid','$name','$amount','$type','0','$bid1','$acyear')";
				...
				header("location:discount_edit.php?cid=$cids&bid=$bid&did=$did&msg=succ");			 
			}
		}else{
				header("location:discount_edit.php?cid=$cids&bid=$bid&did=$did&msg=ferr");
		}	
}
				  ...
				  while($row5=mysql_fetch_array($qry5))
					{
					  ...
					  if($pending>$frate1){
						...
						?>
                                    ...
                                    <td style="vertical-align: middle;">
                                    <?php if($totalpending){?>
                                    <input type="text" name="fees<?=$cartid?>" id="fees<?=$cartid?>" class="biginput txt" id="autocomplete" class="required" value="<?=$eamount?>" autocomplete="off" <?php if($payment=='1'){ echo "readonly"; }?> max="<?=$totalpending?>" /><?php }else{ echo "Fully Paid"; }?>
                                    </td>
                            </tr>
                             <?php $count++; } ?>
                             <?php
				  ...
				  while($row5=mysql_fetch_array($qry5))
					{
					  ...
					  if($pending>$frate1){
						 ...
						 ?>
                                ...
                                <td><?=$pending1?></td>
                                <td><?php if($pending1){?><input type="text" name="fees<?php echo $cartid;?>" id="fees<?php echo $cartid;?>" class="biginput txt" id="autocomplete" class="required" value="<?=$eamount?>" autocomplete="off" max="<?=$pending1?>"/> <?php }else{ echo "Fully Paid"; }?></td>
                             </tr>
                             <?php $count++;} ?>
                             <tr>
							...
							<td class="grand_total">Rs. <span id="fgtotal"><?=number_format($totalamount,2)?></span>
							<input type="hidden" class="medium" name="total" id="finaltotal" value="<?=$totalamount?>"/>
							<input type="hidden"  name="total_pending" value="<?= $allpending?>"/></td></td>
						    </tr>
                             <tr>
