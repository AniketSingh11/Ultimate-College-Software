 ...
 if (isset($_POST['submit']))
{
	...
	$qrytest=mysql_query("SELECT * FROM mfrate WHERE ay_id=$acyear AND b_id=$bid AND c_id=$cid AND rate='$rate' AND fg_id='$fgid' AND fgd_id='$fgdid' AND s_id='$sid'");
			  $already=mysql_fetch_array($qrytest);
	if($already){
		header("Location:mfeesrate.php?cid=$cid&rate=$rate&bid=$bid&msg=err");
	}else{
					$sql=mysql_query("INSERT INTO mfrate (fg_id,fgd_id,rate,c_id,s_id,b_id,ay_id) VALUES
('$fgid','$fgdid','$rate','$cid','$sid','$bid','$acyear')") or die("Could not insert data into DB: " . mysql_error());
                ...
                <?php } ?> 
                 ...
                 <div class="block-border">
					<form id="validate-form" class="block-content form" action="" method="post">
                        ...
                        <select name="fgid1" id="fgid" class="required" onchange="change_feerate()" >
                                    ...
                                    <?php } $qry1=mysql_query("SELECT * FROM mfgroup_detail WHERE otherfees='0'");													
									  while($row1=mysql_fetch_array($qry1))
										{?>
                                    <option value="<?php echo $row1['fgd_id']."-other";?>"><?php echo $row1['name'];?></option>
                                    <?php } ?>?>
                                </select>
                            </p>
                            </div>
...
function change_feerate() { 
      var fg_id =document.getElementById('fgid').value;
	  //alert(fg_id);
	  $.get("mfees_calculate.php",{value:fg_id,cid:<?php if($cid){ echo $cid;}else{ echo "0";}?>,bid:<?php echo $bid;?>,sid:<?php if($sid){ echo $sid;}else{ echo "0";}?>,rate:'<?php echo $rate;?>'},function(data){
			$( "#test" ).html(data);
        });	  
}
</script>
